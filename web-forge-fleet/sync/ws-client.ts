...
export function useDashboardWebSocket(path = '/ws', onEvent?: WsMessageHandler): WsClientState {
  ...
  const resolvedWsUrl = useMemo(
    // Next dev rewrites don't proxy websockets, so in dev this connects
    // directly to the gateway; in the static export it's same-origin.
    () => wsUrl(path),
    [path]
  )
