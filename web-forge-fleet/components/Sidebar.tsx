...
const mainSections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/queue', label: 'Deferred Queue', icon: '⏳' },
      { to: '/jira', label: 'Jira Monitor', icon: '🎫' },
    ],
  },
...
export function Sidebar({ collapsed: collapsedProp }: SidebarProps) {
  ...
  const isSectionActive = (section: NavSection) => {
    if (section.link) return pathname === section.link
    ...
    >
      ...
      <nav className="flex-1 space-y-1">
        {mainSections.map((section) => {
          ...
          if (section.link) {
            const linkActive = pathname === section.link
            return (
              <div key={section.id}>
                <Link
                  href={section.link}
              ...
              {(isExpanded || collapsed) && (
                ...
                >
                  {(section.items ?? []).map((item) => {
                    const linkActive = pathname === item.to || pathname.startsWith(item.to + '/')
                    return (
                      <li key={item.to}>
                        <Link
                          href={item.to}
