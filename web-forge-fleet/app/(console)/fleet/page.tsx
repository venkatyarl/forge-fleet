...
import { useWsEvent } from '@/app/(console)/layout'
    ...
    <section className="min-h-full space-y-6 bg-background">
      ...
      <Card className="bg-surface">
        ...
        {isLoading ? (
          ...
          <EmptyFleetState onPrimary={() => router.push('/onboarding')} />
        ) : (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {nodes.map((node) => (
...
function FleetNodeCard({ node }: { node: FleetComputer }) {
  ...
  return (
    <Link href={`/nodes/${nodeId}`} className="group block rounded-xl focus-visible:outline-hidden">
