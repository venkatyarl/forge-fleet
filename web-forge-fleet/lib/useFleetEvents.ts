...
export function useFleetEvents(handler: FleetEventHandler): { live: boolean } {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      if (cancelled) return
      try {
        // In dev, Next rewrites don't proxy SSE, so this connects directly
        // to the gateway (see lib/gateway.ts); in the static export it's
        // same-origin.
        es = new EventSource(sseUrl('/api/events/stream'))
      } catch {
        setLive(false)
        return
      }
