...
impl BrainStateClient {
    ...
    pub async fn stack_push(
        ...
        if len > STACK_CAP {
            let overflow: Vec<String> = self
                .redis
                .lrange(&key, STACK_CAP as isize, -1)
                .await
                .map_err(|e| format!("lrange overflow: {e}"))?;
            let _: () = self
                .redis
                .ltrim(&key, 0, (STACK_CAP - 1) as isize)
                .await
                .map_err(|e| format!("ltrim: {e}"))?;
            for raw in &overflow {
    ...
    pub async fn stack_pop(
        ...
        match raw {
            Some(json) => {
                ...
                let _ = self
                    .archive_stack_item_popped(user_id, Some(thread_id), &item)
                    .await;
                let event = serde_json::json!({
                    "kind": "stack_pop",
                    "user": user_id.to_string(),
    ...
    async fn archive_stack_item_popped(
        &self,
        user_id: &Uuid,
        thread_id: Option<&Uuid>,
        item: &StackItem,
    ...
    pub async fn backlog_add(
        ...
        let count: usize = self
            ...
            .zcard(&key)
            .await
            .map_err(|e| format!("zcard: {e}"))?;
        if count > BACKLOG_CAP {
            let overflow: Vec<String> = self
                .redis
                .zrange(&key, BACKLOG_CAP as isize, -1)
                .await
                .map_err(|e| format!("zrange overflow: {e}"))?;
            let _: () = self
                .redis
                .zremrangebyrank(&key, BACKLOG_CAP as isize, -1)
                .await
                .map_err(|e| format!("zremrangebyrank: {e}"))?;
            for raw in &overflow {
                if let Ok(bi) = serde_json::from_str::<BacklogItem>(raw) {
                    let _ = self
                        .archive_backlog_completed(user_id, project, &bi, "overflow")
                        .await;
                }
            }
        }
