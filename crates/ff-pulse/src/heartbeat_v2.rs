...
\tether 02:42:00:00:00:01\n";
    ...
    #[test]
    fn link_mac_parser_tolerates_invalid_utf8_command_output() {
        let accepted = std::collections::HashSet::from(["eth0"]);
        let bytes = b"\xff\n2: eth0: <UP> mtu 1500 link/ether AA:BB:CC:DD:EE:FE\n";
        assert_eq!(
            parse_link_macs(&String::from_utf8_lossy(bytes), &accepted),
            vec!["aa:bb:cc:dd:ee:fe"]
        );
    }

    #[test]
    fn virtual_interface_filter_rejects_prefixed_names() {
        for iface in [
            "vethabc",
            "br-fleet",
            "virbr0",
            ...
            assert!(
                is_virtual_iface(iface),
                "accepted virtual interface {iface}"
            );
        }
        for iface in ["eth0", "en0", "wlan0", "bond0"] {
            assert!(
                !is_virtual_iface(iface),
                "rejected physical interface {iface}"
            );
        }
    }
}
