//! Multi-Spark sharding map for Qwen3-Coder-480B.
//!
//! Qwen3-Coder-480B-A35B is far too large to serve from a single DGX Spark: the
//! Q4-quantized weights are ~276 GB, well beyond one Spark's unified memory. To
//! serve it locally we partition the model's transformer decoder layers across a
//! **ring of 3 Sparks** and stream activations around the ring (pipeline / ring
...
//!   ([`RingLink`]),
//! - the Q4 footprint accounting so a caller can check each shard fits.
//!
//! It intentionally holds no process or I/O state — it is the *plan* a launcher
//! (e.g. a vLLM `--pipeline-parallel-size` deployment) consumes.
...
/// Number of DGX Sparks the 480B model is partitioned across.
pub const SPARK_RING_SIZE: usize = 3;

/// Weight quantization scheme, used to reason about a shard's footprint.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Quant {
    /// 4-bit weights — the profile this ring is sized for (~276 GB total).
    Q4,
    /// 8-bit weights (~2x Q4).
    Q8,
...
impl Quant {
    /// Short catalog-style tag (`"q4"`, `"q8"`, `"f16"`).
