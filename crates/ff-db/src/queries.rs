...
/// A fleet node row from the Postgres `fleet_workers` table, LEFT-JOINed
/// with `computers` for physical hardware. Merges what used to be separate
/// `Computer` and `FleetWorker` concepts into the single [`crate::models::FleetNode`]
/// entity; kept as a type alias here since this name is widely used by callers.
pub type FleetNodeRow = crate::models::FleetNode;
...
/// Exact catalog workload tags that classify a model as code-capable.
/// Keep this as the single authority for every production query below.
pub const CODE_WORKLOAD_SYNONYMS: &[&str] = &[
    "code",
    "code-gen",
    "codegen",
    "coder",
...
#[cfg(test)]
fn is_code_workload(workload: &str) -> bool {
    CODE_WORKLOAD_SYNONYMS.contains(&workload)
}

#[cfg(test)]
mod code_workload_tests {
    use super::*;

    #[test]
    fn exact_synonyms_and_near_misses() {
        for synonym in CODE_WORKLOAD_SYNONYMS {
            assert!(is_code_workload(synonym), "missing synonym {synonym}");
        }
        for near_miss in ["codec", "code gen", "codes", "reviewing", "Code", ""] {
            assert!(
                !is_code_workload(near_miss),
                "accepted near miss {near_miss}"
            );
        }
    }

    #[tokio::test]
    async fn postgres_exact_match_is_fail_closed_for_non_arrays() {
        ...
        for synonym in CODE_WORKLOAD_SYNONYMS {
            let matched: bool = sqlx::query_scalar(
                "SELECT EXISTS (SELECT 1 FROM jsonb_array_elements_text(jsonb_build_array($1::text)) w WHERE w = ANY($2::text[]))",
            )
            .bind(synonym)
            .bind(CODE_WORKLOAD_SYNONYMS)
            .fetch_one(&pool)
            .await
            .expect("classify synonym");
            assert!(matched, "database missed {synonym}");
        }
        for value in [serde_json::Value::Null, serde_json::json!({"code": true})] {
            let matched: bool = sqlx::query_scalar(
                "SELECT CASE WHEN jsonb_typeof($1::jsonb) = 'array' THEN EXISTS (SELECT 1 FROM jsonb_array_elements_text($1::jsonb) w WHERE w = ANY($2::text[])) ELSE FALSE END",
            )
            .bind(value)
            .bind(CODE_WORKLOAD_SYNONYMS)
            .fetch_one(&pool)
            .await
            .expect("classify malformed workloads");
            assert!(!matched);
        }
    }
}
...
pub async fn pg_supplied_slots_by_kind(pool: &PgPool, min_ctx: i32) -> Result<ServingSupply> {
    let rows = sqlx::query(
        ...
        SELECT d.id              AS id,
               ...
               EXTRACT(EPOCH FROM (NOW() - d.last_health_at))::int AS health_age_sec,
               (CASE WHEN jsonb_typeof(cat.preferred_workloads) = 'array' THEN EXISTS (
                    SELECT 1 FROM jsonb_array_elements_text(cat.preferred_workloads) workload
                     WHERE workload = ANY($2::text[])
                ) ELSE FALSE END) AS is_code,
    ...
    let row = sqlx::query(
        ...
        SELECT lib.id          AS lib_id,
         ...
         WHERE lib.worker_name = $1
           AND cat.tool_calling = TRUE
           AND lib.pinned = TRUE
           AND ($2 = (CASE WHEN jsonb_typeof(cat.preferred_workloads) = 'array' THEN EXISTS (
                    SELECT 1 FROM jsonb_array_elements_text(cat.preferred_workloads) workload
                     WHERE workload = ANY($3::text[])
                ) ELSE FALSE END))
           AND NOT EXISTS (
...
const DEFERRED_TASK_SELECT: &str = r#"
SELECT
    t.id,
    t.created_at,
...
pub async fn pg_list_deferred(
    ...
    let rows = if let Some(s) = status {
        sqlx::query(&format!(
            "{DEFERRED_TASK_SELECT} AND t.status = $1 ORDER BY t.created_at DESC LIMIT $2"
        ))
        .bind(s)
        .bind(limit)
        .fetch_all(pool)
        ...
        sqlx::query(&format!(
            "{DEFERRED_TASK_SELECT} ORDER BY t.created_at DESC LIMIT $1"
        ))
        .bind(limit)
        .fetch_all(pool)
        .await?
...
pub async fn pg_deferred_stats(pool: &PgPool, window_hours: i64) -> Result<DeferredStats> {
    ...
    let recent_failures = sqlx::query(
        "SELECT LEFT(COALESCE(error, '(no error text)'), 60) AS label, COUNT(*) AS count
           FROM fleet_tasks
          WHERE task_class = 'deferred'
            AND status = 'failed'
    ...
    let recent_created = sqlx::query(
        "SELECT split_part(summary, ' ', 1) || ' ' || split_part(summary, ' ', 2) AS label,
                COUNT(*) AS count
           FROM fleet_tasks
          WHERE task_class = 'deferred'
            AND created_at > now() - make_interval(hours => $1::int)
...
pub async fn pg_get_deferred(pool: &PgPool, id: &str) -> Result<Option<DeferredTaskRow>> {
    let uuid = sqlx::types::Uuid::parse_str(id)
        .map_err(|e| crate::error::DbError::NotFound(format!("bad uuid {id}: {e}")))?;
    let row = sqlx::query(&format!("{DEFERRED_TASK_SELECT} AND t.id = $1"))
        .bind(uuid)
        .fetch_optional(pool)
        .await?;
...
pub async fn pg_claim_deferred(
    ...
    let claimed = if let Some(r) = row {
        ...
        sqlx::query(
            "UPDATE fleet_tasks
                SET status = 'running',
                    payload = jsonb_set(payload, '{claimed_by}', to_jsonb($1::text), true),
                    claimed_at = NOW()
              WHERE id = $2
                AND task_class = 'deferred'",
        )
...
mod tests {
    ...
    async fn create_temp_db() -> Option<(PgPool, PgPool, String)> {
        ...
        .execute(&pool)
        .await
        .expect("create minimal deferred fold schema");
    ...
    async fn deferred_fold_backfills_view_and_claims_from_fleet_tasks() {
        ...
        sqlx::query(
            "INSERT INTO deferred_tasks
                (id, created_at, created_by, title, kind, payload, trigger_type,
                 trigger_spec, preferred_node, required_caps, status, attempts,
                 max_attempts, next_attempt_at)
             VALUES
                ($1, NOW() - INTERVAL '10 minutes', 'leader:atlas', 'sample deferred task',
                 'shell', '{\"command\":\"echo hi\"}'::jsonb, 'now', '{}'::jsonb,
                 'atlas', '[\"ssh\"]'::jsonb, 'dispatchable', 2, 7, NOW() - INTERVAL '5 minutes')",
        )
        .bind(task_id)
        .execute(&pool)
        .await
        .expect("insert legacy deferred row");

        sqlx::raw_sql(crate::schema::SCHEMA_V159_FOLD_DEFERRED_TASKS)
            .execute(&pool)
            .await
            .expect("run deferred fold migration");

        let row = sqlx::query(
            "SELECT created_by, title, kind, payload, trigger_type, preferred_node,
                    required_caps, status, attempts, max_attempts, next_attempt_at
               FROM deferred_tasks
              WHERE id = $1",
        )
        .bind(task_id)
        .fetch_one(&pool)
        .await
        .expect("read deferred compat view");

        assert_eq!(
            row.get::<Option<String>, _>("created_by").as_deref(),
            Some("leader:atlas")
        );
        assert_eq!(row.get::<String, _>("title"), "sample deferred task");
        assert_eq!(row.get::<String, _>("kind"), "shell");
        assert_eq!(
            row.get::<serde_json::Value, _>("payload"),
            serde_json::json!({ "command": "echo hi" })
        );
        assert_eq!(row.get::<String, _>("trigger_type"), "now");
        assert_eq!(
            row.get::<Option<String>, _>("preferred_node").as_deref(),
            Some("atlas")
        );
        assert_eq!(row.get::<String, _>("status"), "dispatchable");
        assert_eq!(row.get::<i32, _>("attempts"), 2);
        assert_eq!(row.get::<i32, _>("max_attempts"), 7);
        assert_eq!(
            row.get::<serde_json::Value, _>("required_caps"),
            serde_json::json!(["ssh"])
        );

        let claimed = pg_claim_deferred(&pool, "atlas")
            .await
            .expect("claim deferred task")
            .expect("one deferred task claimed");
        assert_eq!(claimed.id, task_id.to_string());
        assert_eq!(claimed.preferred_node.as_deref(), Some("atlas"));

        let remaining: i64 = sqlx::query_scalar(
            "SELECT COUNT(*)
               FROM fleet_tasks
              WHERE task_class = 'deferred'
                AND status = 'dispatchable'",
        )
        .fetch_one(&pool)
        .await
