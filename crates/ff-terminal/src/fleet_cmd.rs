...
async fn live_remote_main_sha() -> Result<String> {
    let output = tokio::process::Command::new("git")
        ...
        .output()
        .await
        .map_err(|e| anyhow::anyhow!("failed to run git ls-remote origin refs/heads/main: {e}"))?;
...
pub async fn handle_fleet_versions_live(
    ...
    for n in nodes {
        ...
        futs.push(async move {
            // The checked-out SHA (`rev-parse HEAD`) AND the RUNNING process SHA
            ...
            let (head, health) = raw.split_once("@@H@@").unwrap_or((raw.as_str(), ""));
            let running_sha = extract_health_build_sha(health.trim());
            (name, head.trim().to_string(), running_sha)
        });
    }
    ...
    while let Some((name, raw, running_sha)) = futs.next().await {
        let parsed = if raw.len() == 40 && raw.bytes().all(|b| b.is_ascii_hexdigit()) {
            Some(BuildVersion {
                date: "-".into(),
                build_count: 0,
                state: "-".into(),
...
fn deploy_playbook(os_family: &str, source_tree_path: &str, target_sha: &str) -> String {
    ...
    // before building anything. Linux trees accumulate build
...
async fn verify_deploy_convergence(
    ...
    match BuildVersion::parse(&raw) {
        Some(v) if target_sha.starts_with(&v.sha) || v.sha.starts_with(target_sha) => {
            DeployResult {
                name: t.name,
                ok: true,
...
async fn handle_fleet_deploy(
    ...
    // Convergence target is the live ls-remote SHA pinned before rollout, not
    // a modal result that could bless a fleet uniformly stuck on stale code.
    let target_short: String = target_sha.chars().take(8).collect();
    let converged = results
        .iter()
        .filter(|r| r.ok && r.sha == target_short)
        .count();
    ...
    let target_disp = target_short.as_str();
    println!();
...
mod route_tests {
    ...
    fn versions_live_status_cases() {
        ...
        assert_eq!(
            versions_live_status("aaaaaaaa", Some("aaaaaaaa"), Some("aaaaaaaa")),
            "converged"
        );
        // 2. disk matches target but running does not (running is an OLD sha) -> "stale-daemon"
        assert_eq!(
            versions_live_status("bbbbbbbb", Some("aaaaaaaa"), Some("bbbbbbbb")),
            "stale-daemon"
        );
        // 3. disk does NOT match target -> "drift"
        assert_eq!(
            versions_live_status("aaaaaaaa", Some("bbbbbbbb"), Some("cccccccc")),
            "drift"
        );
        // 4. target is None -> "unknown"
        assert_eq!(
            versions_live_status("aaaaaaaa", Some("aaaaaaaa"), None),
            "unknown"
        );
        // 5. running is None (gateway down) but disk matches target -> "stale-daemon"
        assert_eq!(
            versions_live_status("bbbbbbbb", None, Some("bbbbbbbb")),
            "stale-daemon"
        );
    }
    ...
    fn deploy_playbook_refuses_dirty_tree_before_reset_hard() {
        ...
        let p = super::deploy_playbook("macos", "~/projects/forge-fleet", target);
        let dirty = p
        ...
        let fetch = p
            ...
            .expect("must fetch the explicit live target");
        let verify = p
            .find("[ \"$ACTUAL_SHA\" =")
            .expect("must verify post-reset HEAD");
        assert!(fetch < reset && reset < verify);
