...
pub async fn handle_status(pg: &PgPool) -> Result<()> {
    ...
    if rows.is_empty() {
        ...
        println!("{:<18} {:<4} {:<14} {:>6} {:>5}", "SIG", "TIER", "STATUS", "CNT", "TRY");
        for r in rows {
            ...
            println!("{:<18} {:<4} {:<14} {:>6} {:>5}",
                     ...
                     tier, status.chars().take(14).collect::<String>(), cnt, try_);
        }
    }
    println!();
    ...
    if trust_rows.is_empty() {
        ...
        println!("{:<12} {:<4} {:<20} {:>5} {:>5}", "COMPUTER", "TIER", "LEVEL", "CLEAN", "REV");
        for r in trust_rows {
            ...
            println!("{:<12} {:<4} {:<20} {:>5} {:>5}",
                     ...
                     tier, lvl.chars().take(20).collect::<String>(), clean, rev);
        }
    }
    Ok(())
}
...
pub async fn handle_freeze_tier(pg: &PgPool, tier: &str, hours: u32) -> Result<()> {
    ...
    sqlx::query(
        "UPDATE daemon_trust_scores SET probation_until = $1 WHERE tier = $2"
    ).bind(until).bind(tier).execute(pg).await?;
    println!("Tier {} frozen until {} (human approval required)", tier, until);
    Ok(())
}
...
pub async fn handle_trust_reset(pg: &PgPool, computer: &str) -> Result<()> {
    ...
    ).bind(computer).execute(pg).await?;
    println!("Trust reset for {}: back to operator-approve probation", computer);
    Ok(())
}