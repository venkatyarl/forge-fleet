...
pub async fn handle_daemon(
    _as_node: Option<String>,
    _scheduler: bool,
    _defer_interval: u64,
    _disk_interval: u64,
    _reconcile_interval: u64,
    _once: bool,
) -> Result<()> {
    // Phase C of the legacy-daemon retirement (decision-2): the actuating
    // loop is gone — forgefleetd (src/main.rs) owns every background tick
    // (scheduler, defer-worker, disk sampler, reconciler, sweeper, version
    // check, mesh refresh/repair, model scan/upgrade, vault re-index, github
    // sync, oauth probe, …). `ff daemon` survives only as a no-op stub so any
    // lingering script or service unit that still invokes it exits cleanly
    // instead of erroring. Run `ff defer-worker` for a standalone defer loop.
    eprintln!(
        "legacy `ff daemon` is retired — forgefleetd now owns all background \
         ticks. This command is a no-op; remove it from any scripts or service \
         units (use `ff defer-worker` if you need a standalone defer loop)."
    );
    Ok(())
}
