...
pub async fn handle_queue() -> Result<()> {
    ...
    let fleet_class_rows = sqlx::query(
    ...
    .fetch_all(&pool)
    .await
    .context("query fleet_tasks task_class counts")?;

    let mut fleet_class_counts =
        std::collections::BTreeMap::<String, std::collections::HashMap<String, i64>>::new();
    for row in fleet_class_rows {
    ...
    for task_class in FLEET_TASK_CLASSES {
        let counts = fleet_class_counts.get(*task_class);
        println!(
            "{:<12} {:>8} {:>8} {:>10} {:>8}",
            task_class,
            counts.and_then(|m| m.get("pending")).copied().unwrap_or(0),
            counts.and_then(|m| m.get("running")).copied().unwrap_or(0),
            counts
                .and_then(|m| m.get("completed"))
                .copied()
                .unwrap_or(0),
            counts.and_then(|m| m.get("failed")).copied().unwrap_or(0)
        );
    }
