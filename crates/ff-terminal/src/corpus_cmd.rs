...
pub async fn handle_corpus(pool: &PgPool, cmd: crate::CorpusCommand) -> Result<()> {
    match cmd {
        crate::CorpusCommand::Add {
            ...
            let pairs: Vec<(String, Option<String>)> = roots
                .iter()
                .enumerate()
                .map(|(i, r)| (r.clone(), labels.get(i).cloned()))
                .collect();
            ...
            println!(
                "  ({} source root(s) unified under one corpus)",
                sources.len()
            );
        }
        ...
        crate::CorpusCommand::Candidates {
            slug,
            status,
            kind,
            format,
            ...
            let c = require_corpus(pool, &slug).await?;
            let rows =
                corpus::list_candidates(pool, &c, status.as_deref(), kind.as_deref()).await?;
            if format == "json" {
                let v: Vec<_> = rows
                    .iter()
                    .map(|r| {
                ...
                for r in &rows {
                    println!(
                        ...
                        r.kind,
                        r.confidence,
                        r.heuristic.as_deref().unwrap_or("-"),
                        r.title
                    );
                }
            }
        }
        crate::CorpusCommand::Confirm {
            slug,
            candidates,
            all,
        } => {
...
pub async fn handle_query(
    ...
    for r in roles {
        q.facets.push(("role".into(), r.clone()));
    }
    for s in statuses {
        q.facets.push(("status".into(), s.clone()));
    }
    for m in modalities {
        q.facets.push(("modality".into(), m.clone()));
    }
    for f in facets {
...
async fn require_corpus(pool: &PgPool, slug: &str) -> Result<corpus::Corpus> {
    corpus::get_corpus(pool, slug).await?.ok_or_else(|| {
        anyhow!("no corpus with slug '{slug}' (create it with `ff brain corpus add`)")
    })
}
