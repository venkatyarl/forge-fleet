...
enum Command {
    ...
    /// Show provider usage headroom across the fleet.
    Usage {
        /// Emit JSON instead of a table.
        #[arg(long)]
        json: bool,
...
pub enum CortexCommand {
    ...
    /// Inspect functions that read one config/env/secret/feature-flag key.
    ConfigKey {
        name: String,
        #[arg(long)]
        corpus: Option<String>,
        ...
        Some(Command::Oauth { command }) => {
            ...
            match command {
                ...
                OauthCommand::Refresh { provider } => {
                    let providers = resolve(&provider)?;
                    for p in providers {
                        match ff_agent::oauth_distributor::refresh_and_redistribute(&pool, p).await
                        {
                            Ok(n) => println!(
                                "{:<10} {GREEN}✓{RESET} refreshed, imported, and enqueued {n} re-push task(s)",
                                p.name
                            ),
                            Err(e) => println!("{:<10} {RED}✗{RESET} refresh: {e}", p.name),
                        }
                    }
                    Ok(())
                }
