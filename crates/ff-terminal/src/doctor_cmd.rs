...
async fn collect_error_report(pool: &sqlx::PgPool) -> Result<ErrorReport> {
    ...
    // ErrorMiner lifecycle work adds the timestamps/states used by MTTR and
    // resolution rate. During rolling deploys the base table may briefly exist
    // before those display fields; doctor must remain usable in that window.
