...
impl AgentTool for ReminderTool {
    ...
    fn description(&self) -> &str { "Set a reminder — saves a note with a target time. Reminders are stored and checked by the cron system." }
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let minutes = input.get("in_minutes").and_then(Value::as_u64).unwrap_or(30);
        let target = chrono::Utc::now() + chrono::Duration::minutes(minutes as i64);
        let reminders_dir = dirs::home_dir().unwrap_or_default().join(".forgefleet").join("reminders");
        ...
        let _ = tokio::fs::write(&path, serde_json::to_string_pretty(&reminder).unwrap_or_default()).await;
        AgentToolResult::ok(format!("Reminder set: \"{message}\" at {}", target.format("%H:%M UTC (%B %d)")))
    }
}
...
impl AgentTool for TimerTool {
    ...
    fn description(&self) -> &str { "Time how long a command takes. Useful for benchmarking." }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let label = input.get("label").and_then(Value::as_str).unwrap_or("Timer");
        ...
        match output {
            Ok(o) => {
                ...
                AgentToolResult::ok(format!("{label}: {:.2}s ({}ms)\nExit: {}\n\n{stdout}", elapsed.as_secs_f64(), elapsed.as_millis(), o.status.code().unwrap_or(-1)))
            }
...
impl AgentTool for TimezoneConvertTool {
    fn name(&self) -> &str { "TimezoneConvert" }
    fn description(&self) -> &str { "Convert time between timezones. Also shows current time in multiple zones." }
    fn parameters_schema(&self) -> Value {
        ...
        match action {
            "now" => {
                ...
                match Command::new("bash").arg("-c").arg(cmd).output().await {
                    Ok(o) => AgentToolResult::ok(format!("Current times:\n{}", String::from_utf8_lossy(&o.stdout))),
                    Err(e) => AgentToolResult::err(format!("Failed: {e}")),
                }
            }
            "convert" => {
                let time = input.get("time").and_then(Value::as_str).unwrap_or("now");
                let from = input.get("from_tz").and_then(Value::as_str).unwrap_or("UTC");
                let to = input.get("to_tz").and_then(Value::as_str).unwrap_or("US/Eastern");
                let cmd = format!("TZ='{}' date -d 'TZ=\"{}\" {}' '+%Y-%m-%d %H:%M %Z' 2>/dev/null || echo 'Use: TimezoneConvert now for current times'", to, from, time);
                match Command::new("bash").arg("-c").arg(&cmd).output().await {
                    Ok(o) => AgentToolResult::ok(format!("{time} {from} = {}", String::from_utf8_lossy(&o.stdout).trim())),
...
impl AgentTool for RegexTool {
    ...
    fn description(&self) -> &str { "Test regex patterns against input text. Shows matches, capture groups, and explains the pattern." }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let action = input.get("action").and_then(Value::as_str).unwrap_or("test");
        // Use grep/sed for regex operations
        match action {
            "test" => {
                let cmd = format!("echo '{}' | grep -oP '{}' | head -20", text.replace('\'', "'\"'\"'"), pattern.replace('\'', "'\"'\"'"));
                match Command::new("bash").arg("-c").arg(&cmd).output().await {
                    Ok(o) => {
                        let matches = String::from_utf8_lossy(&o.stdout);
                        if matches.trim().is_empty() { AgentToolResult::ok(format!("No matches for /{pattern}/")) }
                        else { AgentToolResult::ok(format!("Matches for /{pattern}/:\n{matches}")) }
                    }
                    Err(e) => AgentToolResult::err(format!("Regex test failed: {e}")),
                }
            }
            "replace" => {
                let replacement = input.get("replacement").and_then(Value::as_str).unwrap_or("");
                let cmd = format!("echo '{}' | sed -E 's/{}/{}/g'", text.replace('\'', "'\"'\"'"), pattern.replace('/', "\\/"), replacement.replace('/', "\\/"));
                match Command::new("bash").arg("-c").arg(&cmd).output().await {
                    Ok(o) => AgentToolResult::ok(format!("Result: {}", String::from_utf8_lossy(&o.stdout).trim())),
...
impl AgentTool for DiagramTool {
    ...
    fn description(&self) -> &str { "Generate Mermaid diagrams from descriptions. Outputs Mermaid syntax that can be rendered in markdown." }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        let diagram_type = input.get("type").and_then(Value::as_str).unwrap_or("flowchart");
...
impl AgentTool for TranslateTool {
    ...
    fn description(&self) -> &str { "Translate text between languages. Uses the fleet LLM for translation (no external API needed)." }
    fn parameters_schema(&self) -> Value {
...
impl AgentTool for FileCompressTool {
    fn name(&self) -> &str { "FileCompress" }
    fn description(&self) -> &str { "Compress and decompress files: zip, tar.gz, tar.bz2. Create archives from files/directories." }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let format = input.get("format").and_then(Value::as_str).unwrap_or("tar.gz");

        let cmd = match action {
            "compress" => {
                let out = if output_path.is_empty() { format!("{input_path}.{}", format.replace('.', "")) } else { output_path.to_string() };
                match format {
            ...
            "decompress" => {
                if input_path.ends_with(".zip") { format!("unzip '{input_path}'") }
                else if input_path.ends_with(".bz2") { format!("tar -xjf '{input_path}'") }
                else { format!("tar -xzf '{input_path}'") }
            }
            "list" => {
                if input_path.ends_with(".zip") { format!("unzip -l '{input_path}'") }
                else { format!("tar -tzf '{input_path}' | head -50") }
            }
...
impl AgentTool for FileSyncTool {
    fn name(&self) -> &str { "FileSync" }
    fn description(&self) -> &str { "Sync files between local and fleet nodes using rsync. Supports bidirectional sync." }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let dest = input.get("destination").and_then(Value::as_str).unwrap_or("");
        let dry_run = input.get("dry_run").and_then(Value::as_bool).unwrap_or(false);
        let excludes: Vec<&str> = input.get("exclude").and_then(Value::as_array)
            .map(|a| a.iter().filter_map(Value::as_str).collect()).unwrap_or_default();

        let mut args = vec!["-avz", "--progress"];
        if dry_run { args.push("--dry-run"); }
        for exc in &excludes { args.push("--exclude"); args.push(exc); }
        args.push(source);
        args.push(dest);
        ...
        match Command::new("rsync").args(&args).output().await {
            Ok(o) => {
                ...
                if o.status.success() { AgentToolResult::ok(truncate_output(&output, MAX_TOOL_RESULT_CHARS)) }
                else { AgentToolResult::err(truncate_output(&output, MAX_TOOL_RESULT_CHARS)) }
            }
...
impl AgentTool for HealthMonitorTool {
    fn name(&self) -> &str { "HealthMonitor" }
    fn description(&self) -> &str { "Check health of URLs, services, and fleet endpoints. Returns status, response time, and content validation." }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        let urls: Vec<&str> = input.get("urls").and_then(Value::as_array)
            .map(|a| a.iter().filter_map(Value::as_str).collect()).unwrap_or_default();
        let expect_status = input.get("expect_status").and_then(Value::as_u64).unwrap_or(200) as u16;
        let expect_content = input.get("expect_content").and_then(Value::as_str);
        let timeout = input.get("timeout_secs").and_then(Value::as_u64).unwrap_or(5);
        ...
        for url in &urls {
            ...
            match client.get(*url).send().await {
                Ok(resp) => {
                    ...
                    let icon = if status_ok && content_ok { "✓" } else { "✗" };
                    results.push(format!("  {icon} {url} — HTTP {status} ({elapsed}ms){}",
                        if !content_ok { " [content mismatch]" } else { "" }));
                }
                Err(e) => {
                    let elapsed = start.elapsed().as_millis();
        ...
        AgentToolResult::ok(format!("Health Check ({} URLs):\n{}", urls.len(), results.join("\n")))
    }
}
...
impl AgentTool for GithubIssuesTool {
    fn name(&self) -> &str { "GithubIssues" }
    fn description(&self) -> &str { "Create, list, view, and manage GitHub issues via gh CLI." }
    fn parameters_schema(&self) -> Value {
        ...
        match action {
            "create" => {
                ...
                if let Some(labels) = input.get("labels").and_then(Value::as_array) {
                    for l in labels.iter().filter_map(Value::as_str) { cmd.args(["--label", l]); }
                }
            }
            "list" => { cmd.args(["issue", "list"]); }
            "view" => {
                if let Some(n) = input.get("issue_number").and_then(Value::as_u64) { cmd.args(["issue", "view", &n.to_string()]); }
                else { return AgentToolResult::err("'issue_number' required for view"); }
            }
            "close" => {
                if let Some(n) = input.get("issue_number").and_then(Value::as_u64) { cmd.args(["issue", "close", &n.to_string()]); }
                else { return AgentToolResult::err("'issue_number' required for close"); }
            }
            "comment" => {
                let body = input.get("body").and_then(Value::as_str).unwrap_or("");
                if let Some(n) = input.get("issue_number").and_then(Value::as_u64) { cmd.args(["issue", "comment", &n.to_string(), "--body", body]); }
                else { return AgentToolResult::err("'issue_number' required for comment"); }
            }
            _ => return AgentToolResult::err(format!("Unknown action: {action}")),
        }
...
impl AgentTool for MarkdownTool {
    fn name(&self) -> &str { "MarkdownConvert" }
    fn description(&self) -> &str { "Convert markdown to HTML or render markdown files. Also validates markdown syntax." }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        match action {
            "to_html" => {
                ...
                let cmd = format!("echo '{}' | pandoc -f markdown -t html 2>/dev/null || python3 -c \"import markdown; print(markdown.markdown(open('/dev/stdin').read()))\" 2>/dev/null || echo 'Install pandoc or python markdown'", text.replace('\'', "'\"'\"'"));
                match Command::new("bash").arg("-c").arg(&cmd).output().await {
