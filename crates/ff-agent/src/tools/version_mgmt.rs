...
impl AgentTool for VersionManagerTool {
    fn name(&self) -> &str { "VersionManager" }
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        match action {
            "current" => {
                ...
                let git_hash = Command::new("git").args(["rev-parse", "--short", "HEAD"])
                    .current_dir(&ctx.working_dir).output().await.ok()
                ...
                let commit_count = Command::new("git").args(["rev-list", "--count", "HEAD"])
                    .current_dir(&ctx.working_dir).output().await.ok()
                    .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string())
                    .unwrap_or_else(|| "?".into());

                let last_commit = Command::new("git").args(["log", "-1", "--format=%s (%ar)"])
                    .current_dir(&ctx.working_dir).output().await.ok()
                    .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string())
                    .unwrap_or_default();
            ...
            "check_update" => {
                ...
                let _fetch = Command::new("git").args(["fetch", "--dry-run"])
                ...
                let remote = Command::new("git").args(["rev-parse", "origin/master"])
                    .current_dir(&ctx.working_dir).output().await.ok()
                    .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string()).unwrap_or_default();
                ...
                if local == remote {
                    ...
                    let behind = Command::new("git").args(["rev-list", "--count", &format!("{local}..origin/master")])
                        .current_dir(&ctx.working_dir).output().await.ok()
                        .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string()).unwrap_or_else(|| "?".into());

                    AgentToolResult::ok(format!("Update available: {behind} commits behind origin/master.\nRun VersionManager upgrade to update."))
                }
            }
            ...
            "upgrade" => {
                ...
                // Step 1: Pull latest
                let pull = Command::new("git").args(["pull", "--rebase"])
                    .current_dir(&ctx.working_dir).output().await;
                match pull {
                    Ok(o) if o.status.success() => steps.push("1. Git pull: OK".into()),
                    Ok(o) => return AgentToolResult::err(format!("Git pull failed:\n{}", String::from_utf8_lossy(&o.stderr))),
            ...
            "deploy_fleet" => {
                ...
                let nodes: Vec<(&str, &str)> = if let Some(node) = target_node {
                    ...
                    vec![
                        ("Marcus", "192.168.5.102"), ("Sophie", "192.168.5.103"),
                        ("Priya", "192.168.5.104"), ("James", "192.168.5.108"),
                    ]
                };

                let binary_path = ctx.working_dir.join("target/release/ff");
                if !binary_path.exists() {
                    return AgentToolResult::err("Binary not found. Run 'VersionManager upgrade' first to build.".to_string());
                }

                let mut results = Vec::new();
                for (name, ip) in &nodes {
                    let scp = Command::new("scp")
                        .args([&binary_path.to_string_lossy().to_string(), &format!("root@{ip}:/usr/local/bin/ff")])
                        .output().await;
                    match scp {
                        Ok(o) if o.status.success() => results.push(format!("  {name} ({ip}): deployed")),
            ...
            "rollback" => {
                let version = input.get("version").and_then(Value::as_str).unwrap_or("HEAD~1");
                let result = Command::new("git").args(["checkout", version, "--", "."])
                    .current_dir(&ctx.working_dir).output().await;
                match result {
                    Ok(o) if o.status.success() => {
                        AgentToolResult::ok(format!("Rolled back to {version}. Rebuild with: cargo build --release -p ff-terminal"))
                    }
            ...
            "changelog" => {
                let since = input.get("version").and_then(Value::as_str).unwrap_or("HEAD~20");
                let output = Command::new("git").args(["log", "--oneline", &format!("{since}..HEAD")])
                    .current_dir(&ctx.working_dir).output().await;
                match output {
                    Ok(o) if o.status.success() => {
                        AgentToolResult::ok(format!("Changelog since {since}:\n\n{}", truncate_output(&String::from_utf8_lossy(&o.stdout), MAX_TOOL_RESULT_CHARS)))
                    }
            ...
            "tag_release" => {
                let channel = input.get("channel").and_then(Value::as_str).unwrap_or("stable");
                ...
                let tag = if !version.is_empty() {
                    ...
                    let base = format!("v{}.{:02}.{:02}", now.format("%Y"), now.format("%m"), now.format("%d"));

                    // Check if today's tag already exists — if so, add patch
                    let existing = Command::new("git").args(["tag", "-l", &format!("{base}*")])
                        .current_dir(&ctx.working_dir).output().await.ok()
                ...
                let result = Command::new("git").args(["tag", "-a", &full_tag, "-m", &message])
                    .current_dir(&ctx.working_dir).output().await;
                match result {
                    Ok(o) if o.status.success() => {
                        AgentToolResult::ok(format!(
                    ...
                    Ok(o) => AgentToolResult::err(format!("Tagging failed: {}", String::from_utf8_lossy(&o.stderr))),
