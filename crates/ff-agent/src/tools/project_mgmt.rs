...
impl AgentTool for ProjectEstimateTool {
    ...
    fn description(&self) -> &str {
        "Estimate effort for tasks. Analyzes description complexity, generates story points (1-13), hour estimates, and confidence levels."
    }
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        AgentToolResult::ok(format!(
            "Estimates ({} tasks):\n{}\n\nTotal: {total_points} points\nAt velocity {velocity}/sprint: ~{sprints_needed:.0} sprints",
            tasks.len(),
            estimates.join("\n")
        ))
    }
...
impl AgentTool for VelocityTrackerTool {
    ...
    fn description(&self) -> &str {
        "Calculate velocity from sprint history. Shows trend, average, and capacity forecast."
    }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        let points: Vec<f64> = input
            ...
            .unwrap_or_default();
        if points.is_empty() {
            return AgentToolResult::err("No sprint data provided");
        }
        let avg = points.iter().sum::<f64>() / points.len() as f64;
        let recent_avg = if points.len() >= 3 {
            points[..3].iter().sum::<f64>() / 3.0
        } else {
            avg
        };
        let trend = if recent_avg > avg {
            "improving"
        ...
        AgentToolResult::ok(format!(
            "Velocity Analysis ({count} sprints):\n  Average: {avg:.1} pts/sprint\n  Recent (3): {recent_avg:.1} pts/sprint\n  Range: {min:.0} - {max:.0}\n  Trend: {trend}\n  Forecast next sprint: {recent_avg:.0} pts",
            count = points.len()
        ))
    }
}
...
impl AgentTool for DeadlineProjectorTool {
    ...
    fn description(&self) -> &str {
        "Project completion date from remaining work and velocity."
    }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let velocity = input
            ...
            .and_then(Value::as_f64)
            .unwrap_or(20.0);
        let sprint_days = input
            .get("sprint_length_days")
            .and_then(Value::as_f64)
            .unwrap_or(14.0);
        if velocity <= 0.0 {
            return AgentToolResult::err("Velocity must be > 0");
        }
...
impl AgentTool for SprintPlannerTool {
    ...
    fn description(&self) -> &str {
        "Auto-assign work items to a sprint based on priority, capacity, and dependencies."
    }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        AgentToolResult::ok(format!(
            "Sprint Plan ({used:.0}/{capacity:.0} pts, {} items):\n{}",
            planned.len(),
            planned.join("\n")
        ))
    }
...
impl AgentTool for RiskAssessorTool {
    ...
    fn description(&self) -> &str {
        "Identify project risks: blocked items, overdue tasks, dependency bottlenecks, scope creep indicators."
    }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        if total > 0.0 && blocked / total > 0.2 {
            risks.push(format!(
                "HIGH: {:.0}% of items are blocked ({blocked:.0}/{total:.0})",
                blocked / total * 100.0
            ));
        }
        if overdue > 0.0 {
            risks.push(format!("MEDIUM: {overdue:.0} overdue items"));
        }
        if original > 0.0 && added / original > 0.3 {
            risks.push(format!(
                "HIGH: Scope creep — {:.0}% increase ({added:.0} items added to {original:.0})",
                added / original * 100.0
            ));
        }
        if risks.is_empty() {
            risks.push("LOW: No significant risks detected".into());
        }
        AgentToolResult::ok(format!(
            "Risk Assessment:\n{}",
            risks
...
impl AgentTool for WorkloadBalancerTool {
    ...
    fn description(&self) -> &str {
        "Distribute work items evenly across assignees/agents based on capacity and current load."
    }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        if assignees.is_empty() {
            return AgentToolResult::err("No assignees provided");
        }
        let mut loads: Vec<(String, Vec<String>, f64)> = assignees
            .iter()
            .map(|a| (a.clone(), Vec::new(), 0.0))
            .collect();
        for item in &items {
            let title = item.get("title").and_then(Value::as_str).unwrap_or("?");
            let points = item.get("points").and_then(Value::as_f64).unwrap_or(3.0);
        ...
        let output: Vec<String> = loads
            .iter()
            .map(|(name, items, total)| format!("  {name} ({total:.0} pts): {}", items.join(", ")))
            .collect();
...
impl AgentTool for DependencyMapperTool {
    ...
    fn description(&self) -> &str {
        "Analyze task dependency chains. Find critical path, circular dependencies, and bottlenecks."
    }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let mut bottlenecks: std::collections::HashMap<String, u32> =
        ...
        for item in &items {
            ...
            let deps: Vec<&str> = item
                ...
                .unwrap_or_default();
            if deps.is_empty() {
                no_deps.push(format!("{id}: {title}"));
            } else {
                has_deps.push(format!("{id}: {title} → depends on [{}]", deps.join(", ")));
                for dep in &deps {
                    *bottlenecks.entry(dep.to_string()).or_insert(0) += 1;
                }
            }
        }
        let mut btn: Vec<_> = bottlenecks.into_iter().collect();
        btn.sort_by(|a, b| b.1.cmp(&a.1));
        let btn_str: Vec<String> = btn
            .iter()
            .take(5)
            .map(|(id, count)| format!("  {id}: blocks {count} items"))
            .collect();
        AgentToolResult::ok(format!(
            "Dependency Analysis:\n\nReady (no deps): {}\n{}\n\nWith dependencies: {}\n{}\n\nBottlenecks:\n{}",
            no_deps.len(),
