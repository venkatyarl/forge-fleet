...
impl AgentTool for DocGenTool {
    fn name(&self) -> &str { "DocGen" }
    fn description(&self) -> &str { "Generate documentation: rustdoc, JSDoc, Python docstrings, or markdown docs from code." }
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        let action = input.get("action").and_then(Value::as_str).unwrap_or("build");
        ...
        if ctx.working_dir.join("Cargo.toml").exists() {
            ...
            match Command::new("cargo").args(&args).current_dir(&ctx.working_dir).output().await {
                Ok(out) => {
                    let combined = format!("{}{}", String::from_utf8_lossy(&out.stdout), String::from_utf8_lossy(&out.stderr));
                    if out.status.success() { AgentToolResult::ok(truncate_output(&combined, MAX_TOOL_RESULT_CHARS)) }
                    else { AgentToolResult::err(truncate_output(&combined, MAX_TOOL_RESULT_CHARS)) }
                }
