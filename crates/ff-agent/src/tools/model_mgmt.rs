...
impl AgentTool for ModelBrowserTool {
    fn name(&self) -> &str { "ModelBrowser" }
    fn description(&self) -> &str { "Search and browse LLM models from HuggingFace, Ollama library, or fleet endpoints. View model cards, sizes, capabilities, and compatibility with fleet hardware." }
        ...
        match action {
                        ...
                        match resp.json::<Vec<Value>>().await {
                            Ok(models) => {
                                ...
                                for (i, model) in models.iter().take(10).enumerate() {
                                    ...
                                    output.push_str(&format!("  {}. {} ({}) — {} downloads, {} likes\n", i + 1, id, pipeline, format_number(downloads), likes));
                                }
            ...
            "list_ollama" => {
                ...
                match Command::new("ollama").arg("list").output().await {
                    Ok(out) if out.status.success() => {
                        AgentToolResult::ok(format!("Ollama Models (local):\n{}", String::from_utf8_lossy(&out.stdout)))
                    }
                    _ => {
                        ...
                        match client.get("https://ollama.com/api/tags").send().await {
                            Ok(resp) if resp.status().is_success() => {
                                AgentToolResult::ok(format!("Ollama: Use 'ollama pull <model>' to download. Popular: qwen2.5-coder, llama3.2, gemma2, codestral, deepseek-coder-v2"))
                            }
                            _ => AgentToolResult::err("Ollama not installed or API unreachable".to_string()),
                        }
                    }
                }
            }
            ...
            "list_fleet" => {
                ...
                let nodes = [
                    ...
                    ("James", "192.168.5.108:51000"), ("James-2", "192.168.5.108:51001"),
                ];
                    ...
                    match client.get(&url).send().await {
                        Ok(resp) if resp.status().is_success() => {
                            if let Ok(data) = resp.json::<Value>().await {
                                let models: Vec<String> = data.get("data").and_then(Value::as_array)
            ...
            "info" => {
                if query.is_empty() { return AgentToolResult::err("'query' (model name) required for info"); }
                        ...
                        match resp.json::<Value>().await {
                            Ok(model) => {
                                ...
                                let tags: Vec<&str> = model.get("tags").and_then(Value::as_array)
                                    .map(|a| a.iter().filter_map(Value::as_str).take(10).collect()).unwrap_or_default();
                                let card = model.get("cardData").and_then(|c| c.get("license")).and_then(Value::as_str).unwrap_or("unknown");

                                AgentToolResult::ok(format!(
                                    "Model: {id}\nPipeline: {pipeline}\nLicense: {card}\nDownloads: {}\nLikes: {likes}\nTags: {}\n\nhttps://huggingface.co/{id}",
                                    format_number(downloads), tags.join(", ")
                                ))
                            }
                            Err(e) => AgentToolResult::err(format!("Parse error: {e}")),
                        }
            ...
            "recommend" => {
                let task = input.get("task").and_then(Value::as_str).unwrap_or("coding");
                let max_ram = input.get("max_ram_gb").and_then(Value::as_u64).unwrap_or(32);

                let recommendations = match task {
                    "coding" => vec![
                        ("Qwen2.5-Coder-32B (Q4_K_M)", 20, "Best coding model for 32GB+ nodes"),
                    ...
                    "vision" => vec![
                        ...
                        ("Llama-3.2-Vision-11B", 8, "Vision-language, fits small nodes"),
                    ],
                    _ => vec![
                ...
                let mut output = format!("Model Recommendations for '{task}' (max {max_ram}GB RAM):\n\n");
...
impl AgentTool for ModelDownloaderTool {
    fn name(&self) -> &str { "ModelDownloader" }
    fn description(&self) -> &str { "Download LLM models to local storage or fleet nodes. Supports Ollama pull, HuggingFace download, and direct GGUF URLs." }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        if model.is_empty() { return AgentToolResult::err("'model' required"); }

        let detected_method = if method != "auto" { method.to_string() }
            else if model.contains(':') || !model.contains('/') { "ollama".into() }
            else if model.starts_with("http") { "url".into() }
            else { "huggingface".into() };

        let cmd = match detected_method.as_str() {
            "ollama" => format!("ollama pull {model}"),
            "huggingface" => {
                let quant = input.get("quantization").and_then(Value::as_str).unwrap_or("Q4_K_M");
...
impl AgentTool for ModelCompareTool {
    fn name(&self) -> &str { "ModelCompare" }
    fn description(&self) -> &str { "Compare LLM models: parameter count, context window, speed, quality benchmarks, RAM requirements, and fleet compatibility." }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        let models: Vec<&str> = input.get("models").and_then(Value::as_array)
            .map(|a| a.iter().filter_map(Value::as_str).collect()).unwrap_or_default();

        if models.len() < 2 { return AgentToolResult::err("Need at least 2 models to compare"); }
        ...
        for model in &models {
            ...
            output.push_str(&format!("| {model} | {params} | {context} | {ram} | {best_for} |\n"));
        }
...
fn estimate_model_specs(model: &str) -> (&str, &str, &str, &str) {
    ...
    if lower.contains("72b") || lower.contains("70b") { ("72B", "32K", "~45GB", "Complex reasoning") }
    else if lower.contains("32b") || lower.contains("34b") { ("32B", "32K", "~20GB", "Coding & analysis") }
    else if lower.contains("27b") || lower.contains("31b") { ("~30B", "32K-262K", "~18GB", "General purpose") }
    else if lower.contains("14b") || lower.contains("16b") { ("14-16B", "32K", "~10GB", "Balanced speed/quality") }
    else if lower.contains("9b") || lower.contains("8b") || lower.contains("7b") { ("7-9B", "32K", "~5GB", "Fast, lightweight") }
    else if lower.contains("3b") { ("3B", "32K", "~2GB", "Ultra-fast, edge") }
    else if lower.contains("405b") { ("405B", "128K", "~250GB", "Maximum quality") }
    else { ("?", "?", "?", "Unknown") }
}
