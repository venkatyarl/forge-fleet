...
impl AgentTool for ToolSearchTool {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        let list_categories = input.get("list_categories").and_then(Value::as_bool).unwrap_or(false);
        ...
        for tool in &tools {
            ...
            if name.contains(&query) || desc.contains(&query) {
                matches.push((tool.name().to_string(), tool.description().to_string(), schema));
            }
        }
...
fn list_all_categories() -> String {
    ...
    for tool in &tools {
        ...
        let category = if desc.contains("git") || name.starts_with("Git") { "Git & Version Control" }
            else if desc.contains("docker") || name.contains("Docker") { "Docker & Containers" }
            else if desc.contains("database") || desc.contains("sql") { "Database" }
            else if desc.contains("task") { "Task Management" }
            else if desc.contains("web") || desc.contains("fetch") || desc.contains("search") { "Web & Research" }
            else if desc.contains("fleet") || desc.contains("ssh") || desc.contains("node") { "Fleet Operations" }
            else if desc.contains("file") || desc.contains("read") || desc.contains("write") || desc.contains("edit") { "File Operations" }
            else if desc.contains("shell") || desc.contains("bash") || desc.contains("command") { "System & Shell" }
            else if desc.contains("cron") || desc.contains("schedule") { "Scheduling" }
            else if desc.contains("model") || desc.contains("llm") { "Model Management" }
            else if desc.contains("agent") || desc.contains("orchestrat") { "Agent & Orchestration" }
            else if desc.contains("image") || desc.contains("video") || desc.contains("media") || desc.contains("photo") || desc.contains("screenshot") { "Media & Vision" }
            else if desc.contains("lint") || desc.contains("quality") || desc.contains("security") { "Code Quality & Security" }
            else if desc.contains("finance") || desc.contains("money") || desc.contains("budget") { "Finance" }
            else if desc.contains("network") || desc.contains("http") || desc.contains("api") { "Networking & APIs" }
            else if desc.contains("crypto") || desc.contains("encrypt") || desc.contains("hash") { "Crypto & Security" }
            else if desc.contains("doc") || desc.contains("notebook") { "Documentation" }
            else { "Utility" };

        categories.entry(category.to_string()).or_default().push(name);
    }

    let mut result = format!("ForgeFleet has {} tools across {} categories:\n\n", tools.len(), categories.len());
    for (category, names) in &categories {
        result.push_str(&format!("**{}** ({}):\n  {}\n\n", category, names.len(), names.join(", ")));
    }
    result.push_str("Use ToolSearch with a query to get full details and parameter schemas for any tool.");
    result
}
