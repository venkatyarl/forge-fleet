    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        match action {
            "create" => {
                ...
                let implementation = input.get("implementation").and_then(Value::as_str).unwrap_or("");

                if struct_name.is_empty() || tool_name.is_empty() || description.is_empty() {
                    return AgentToolResult::err("'name', 'tool_name', and 'description' are required");
                }

                // Build parameter schema
                let params = input.get("parameters").and_then(Value::as_array).cloned().unwrap_or_default();
                ...
                for param in &params {
                    ...
                    let ptype = param.get("param_type").and_then(Value::as_str).unwrap_or("string");
                    let pdesc = param.get("description").and_then(Value::as_str).unwrap_or("");
                    let req = param.get("required").and_then(Value::as_bool).unwrap_or(false);

                    schema_props.push_str(&format!(
                        "                \"{pname}\": {{ \"type\": \"{ptype}\", \"description\": \"{pdesc}\" }},\n"
                    ));
                    if req { required_list.push(format!("\"{pname}\"")); }
                }
                ...
                let source = format!(r#"//! {description}
...
"#, required = required_list.join(", "));
                ...
                let custom_dir = if let Some(ref root) = ff_root {
                    root.join("crates").join("ff-agent").join("src").join("tools")
                } else {
                    dirs::home_dir().unwrap_or_default().join(".forgefleet").join("custom_tools")
                };
                ...
                match fs::write(&file_path, &source).await {
                    Ok(()) => {
                        ...
                        if input.get("rebuild").and_then(Value::as_bool).unwrap_or(false) {
            ...
            "preview" => {
                let struct_name = input.get("name").and_then(Value::as_str).unwrap_or("MyTool");
                let tool_name = input.get("tool_name").and_then(Value::as_str).unwrap_or("MyTool");
                let description = input.get("description").and_then(Value::as_str).unwrap_or("A custom tool");
            ...
            "list_custom" => {
                let custom_dir = dirs::home_dir().unwrap_or_default().join(".forgefleet").join("custom_tools");
                ...
                if let Some(ref root) = ff_root {
                    let tools_dir = root.join("crates").join("ff-agent").join("src").join("tools");
                ...
                AgentToolResult::ok(format!("Tool files ({}):\n{}", tools.len(), tools.join("\n")))
            }

            "delete" => {
                let struct_name = input.get("name").and_then(Value::as_str).unwrap_or("");
                if struct_name.is_empty() { return AgentToolResult::err("'name' required"); }
                let file_name = to_snake_case(struct_name);
                let custom_path = dirs::home_dir().unwrap_or_default().join(".forgefleet").join("custom_tools").join(format!("{file_name}.rs"));
