...
impl AgentTool for ColorPaletteTool {
    ...
    fn description(&self) -> &str {
        "Generate color palettes, convert between hex/rgb/hsl, check WCAG contrast ratios, and suggest accessible color combinations."
    }
            ...
            "generate" => {
                ...
                for i in 1..count {
                    let offset = match style {
                        "monochromatic" => {
                            let new_l = (l + (i as f64 * 15.0) - 30.0).clamp(10.0, 90.0);
                            (h, s, new_l)
                        }
                        "complementary" => {
                            let new_h = (h + 180.0 * (i as f64 / count as f64)) % 360.0;
                            (new_h, s, l)
                        }
                        "analogous" => {
                            let new_h = (h + 30.0 * i as f64 - 60.0 + 360.0) % 360.0;
                            (new_h, s, l)
                        }
                        "triadic" => {
                            let new_h = (h + 120.0 * i as f64) % 360.0;
                            (new_h, s, l)
                        }
                        "warm" => {
                            let new_h = (h + 15.0 * i as f64).clamp(0.0, 60.0);
                            (new_h, s.min(80.0), l)
                        }
                        "cool" => {
                            let new_h = 180.0 + 15.0 * i as f64;
                            (new_h % 360.0, s.min(70.0), l)
                        }
                        _ => {
                            let new_h = (h + 30.0 * i as f64) % 360.0;
                            (new_h, s, l)
                        }
                    };
  ...
  Use ColorPalette contrast to verify your specific combinations."
                    .to_string(),
            ),
...
impl AgentTool for AccessibilityCheckTool {
    ...
    fn description(&self) -> &str {
        "Check HTML/web pages for accessibility issues: missing alt text, color contrast, ARIA roles, heading hierarchy, form labels, keyboard navigation."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let html = if let Some(path) = file {
            let full_path = if std::path::Path::new(path).is_absolute() {
                ...
                ctx.working_dir.join(path).to_string_lossy().to_string()
            };
        ...
        if html.is_empty() {
            return AgentToolResult::err("Could not read HTML content");
        }
        ...
        if img_count > alt_count {
            issues.push(format!(
                "⚠ Missing alt text: {} of {} images lack alt attributes",
                img_count - alt_count,
                img_count
            ));
        }
        ...
        if h1_count == 0 {
            issues.push("⚠ No <h1> found — page should have exactly one <h1>".into());
        }
        if h1_count > 1 {
            issues.push(format!(
                "⚠ Multiple <h1> tags found ({h1_count}) — page should have exactly one"
            ));
        }
        ...
        if !lower.contains("lang=") {
            issues.push("⚠ Missing lang attribute on <html> tag".into());
        }
        ...
        if empty_links > 0 {
            issues.push(format!(
                "⚠ {empty_links} empty/placeholder links (href=\"#\" or href=\"\")"
            ));
        }
        ...
        if aria_count == 0 {
            issues.push(
                "ℹ No ARIA attributes found — consider adding roles for dynamic content".into(),
            );
        }

        // Check viewport
        if !lower.contains("viewport") {
            issues.push("⚠ No viewport meta tag — may not be mobile-friendly".into());
        }

        let score = if issues.is_empty() {
            100
        } else {
            (100.0 - issues.len() as f64 * 12.0).max(0.0) as u32
        };
...
impl AgentTool for ComponentScaffoldTool {
    ...
    fn description(&self) -> &str {
        "Generate UI component boilerplate for React, Vue, or Svelte. Creates component files with TypeScript, props, styles, and tests."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        AgentToolResult::ok(format!(
            "Generated {framework} component: {name}\n\n```{}\n{component}\n```\n\nSave to: src/components/{name}.{}",
            if framework == "react" {
                "tsx"
...
impl AgentTool for ResponsiveTestTool {
    ...
    fn description(&self) -> &str {
        "Test a URL at multiple screen sizes (mobile, tablet, desktop) by taking screenshots at each breakpoint."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        if url.is_empty() {
            return AgentToolResult::err("'url' required");
        }
        ...
        if results.is_empty() {
            AgentToolResult::err(
                "Screenshot capture failed. Install Chrome or Chromium.".to_string(),
            )
        } else {
            AgentToolResult::ok(format!(
...
impl AgentTool for CSSAnalyzerTool {
    ...
    fn description(&self) -> &str {
        "Analyze CSS and Tailwind usage: find unused classes, duplicate styles, color usage, font families, breakpoint distribution, and bundle size."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        match action {
            ...
            "bundle_size" => {
                ...
                match Command::new("bash").arg("-c").arg(&cmd).output().await {
                    Ok(o) => AgentToolResult::ok(format!(
                        "Bundle Size (top files by bytes):\n{}",
                        String::from_utf8_lossy(&o.stdout)
                    )),
                    Err(e) => AgentToolResult::err(format!("Analysis failed: {e}")),
                }
...
impl AgentTool for StyleGuideGenTool {
    ...
    fn description(&self) -> &str {
        "Generate a style guide document from an existing project. Extracts colors, typography, spacing, components, and patterns used."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        // Gather data
        let colors_cmd = format!(
            "grep -roh '#[0-9a-fA-F]\\{{6\\}}' '{}' --include='*.css' --include='*.tsx' --include='*.ts' 2>/dev/null | sort -u | head -20",
            target.display()
        );
        let fonts_cmd = format!(
            "grep -roh 'font-[a-z]*' '{}' --include='*.tsx' --include='*.css' 2>/dev/null | sort -u | head -10",
            target.display()
        );
        let components_cmd = format!(
            "grep -roh 'export function [A-Z][a-zA-Z]*' '{}' --include='*.tsx' 2>/dev/null | sed 's/export function //' | sort -u",
            target.display()
        );

        let colors = Command::new("bash")
            .arg("-c")
            .arg(&colors_cmd)
            .output()
            .await
        ...
        let fonts = Command::new("bash")
            ...
            .map(|o| String::from_utf8_lossy(&o.stdout).to_string())
            .unwrap_or_default();
        let components = Command::new("bash")
            .arg("-c")
            .arg(&components_cmd)
            .output()
            .await
        ...
        let guide = format!(
            ...
            if components.trim().is_empty() {
                "No React components found.".into()
            } else {
                components
                    .lines()
                    .map(|c| format!("- `<{c} />`"))
        ...
        let output_path = ctx.working_dir.join(
            input
                .get("output")
                .and_then(Value::as_str)
                .unwrap_or("STYLE_GUIDE.md"),
        );
