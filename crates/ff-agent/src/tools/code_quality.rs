...
impl AgentTool for CodeComplexityTool {
    fn name(&self) -> &str { "CodeComplexity" }
    fn description(&self) -> &str { "Analyze code complexity: function count, line counts, nesting depth, and file size distribution." }
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let target = if path == "." { ctx.working_dir.clone() } else if std::path::Path::new(path).is_absolute() { std::path::PathBuf::from(path) } else { ctx.working_dir.join(path) };

        // Use cloc or wc as fallback
        let cloc_result = Command::new("cloc").arg("--quiet").arg(&target).output().await;
        match cloc_result {
            Ok(out) if out.status.success() => {
                AgentToolResult::ok(format!("Code Complexity Analysis:\n\n{}", truncate_output(&String::from_utf8_lossy(&out.stdout), MAX_TOOL_RESULT_CHARS - 100)))
            }
            _ => {
                // Fallback: basic file stats
                let ext_filter = lang.map(|l| format!("*.{l}")).unwrap_or_else(|| "*".to_string());
                let cmd = format!("find '{}' -name '{}' -not -path '*/target/*' -not -path '*/node_modules/*' -not -path '*/.git/*' | head -100 | xargs wc -l 2>/dev/null | sort -rn | head -20", target.display(), ext_filter);
                match Command::new("bash").arg("-c").arg(&cmd).output().await {
                    Ok(out) => AgentToolResult::ok(format!("File size analysis (lines):\n\n{}", truncate_output(&String::from_utf8_lossy(&out.stdout), MAX_TOOL_RESULT_CHARS - 100))),
...
impl AgentTool for DuplicateDetectorTool {
    fn name(&self) -> &str { "DuplicateDetector" }
    fn description(&self) -> &str { "Find duplicate or near-duplicate code blocks in a codebase. Identifies copy-paste patterns." }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let result = Command::new("npx").args(["jscpd", "--min-lines", &min_lines.to_string(), "--reporters", "console", &target.to_string_lossy()]).output().await;
        match result {
            Ok(out) if out.status.success() => {
            ...
            _ => {
                ...
                let lang = input.get("language").and_then(Value::as_str).unwrap_or("rs");
                let cmd = format!(
...
impl AgentTool for LogAnalyzerTool {
    fn name(&self) -> &str { "LogAnalyzer" }
    fn description(&self) -> &str { "Parse and analyze log files. Find errors, warnings, patterns, and frequency distributions." }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let cmd = if filter.is_empty() {
            format!("tail -n {} '{}' | sort | uniq -c | sort -rn | head -30", tail, path.display())
        } else {
            format!("tail -n {} '{}' | grep -i '{}' | head -50", tail, path.display(), filter)
        };
        ...
        match Command::new("bash").arg("-c").arg(&cmd).output().await {
            Ok(out) => {
                ...
                let errors = content.lines().filter(|l| l.to_ascii_lowercase().contains("error")).count();
                let warns = content.lines().filter(|l| l.to_ascii_lowercase().contains("warn")).count();

                AgentToolResult::ok(format!(
                    "Log Analysis: {}\n  Total lines: {lines}\n  Errors: {errors}\n  Warnings: {warns}\n\n{}",
                    path.display(),
                    if filter.is_empty() { format!("Most frequent patterns:\n{output}") } else { format!("Filtered ({filter}):\n{output}") }
                ))
            }
