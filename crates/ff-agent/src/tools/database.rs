...
impl AgentTool for DatabaseQueryTool {
    ...
    fn description(&self) -> &str {
        "Run SQL queries against databases. Supports PostgreSQL (psql), SQLite (sqlite3), and MySQL (mysql). Use for data inspection, schema exploration, and data operations."
    }
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        if query.is_empty() || database.is_empty() {
            return AgentToolResult::err("'query' and 'database' required");
        }
            ...
            || lower.contains("delete from") && !lower.contains("where")
        {
            return AgentToolResult::err(
                "Destructive query blocked. Add a WHERE clause or use Bash for explicit operations.",
            );
        }

        let detected = if db_type != "auto" {
            db_type.to_string()
        } else if database.ends_with(".db")
            || database.ends_with(".sqlite")
            || database.ends_with(".sqlite3")
        {
            "sqlite".into()
        } else if database.starts_with("postgres") || database.starts_with("postgresql") {
            "postgres".into()
        } else if database.starts_with("mysql") {
            "mysql".into()
        } else {
            "sqlite".into()
        };
        ...
        let result = match detected.as_str() {
            "sqlite" => {
                let db_path = if std::path::Path::new(database).is_absolute() {
                    database.to_string()
                } else {
                    ctx.working_dir.join(database).to_string_lossy().to_string()
                };
                Command::new("sqlite3")
                    .args(["-header", "-column", &db_path, query])
                    .output()
                    .await
            }
            "postgres" => {
                Command::new("psql")
                    .args([database, "-c", query])
                    .output()
                    .await
            }
            "mysql" => {
                Command::new("mysql")
                    .args(["-e", query, database])
                    .output()
                    .await
            }
        ...
        match result {
            ...
            Ok(out) => AgentToolResult::err(truncate_output(
                &format!(
                    "{}{}",
                    String::from_utf8_lossy(&out.stdout),
