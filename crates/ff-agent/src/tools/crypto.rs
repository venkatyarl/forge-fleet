...
impl AgentTool for HashGeneratorTool {
    ...
    fn description(&self) -> &str {
        "Generate cryptographic hashes: SHA256, SHA512, MD5, SHA1. Hash strings or files."
    }
...
impl AgentTool for TextTransformTool {
    ...
    fn description(&self) -> &str {
        "Transform text: base64 encode/decode, URL encode/decode, JSON format/minify, count words/lines/chars."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input_val: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        let action = input_val
            .get("action")
            .and_then(Value::as_str)
            .unwrap_or("");
        let text = input_val.get("input").and_then(Value::as_str).unwrap_or("");
        if text.is_empty() {
            return AgentToolResult::err("'input' required");
        }
...
impl AgentTool for CalculatorTool {
    ...
    fn description(&self) -> &str {
        "Evaluate math expressions, unit conversions, and calculations. Uses bc or Python for computation."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
