...
impl AgentTool for EnvInfoTool {
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        let section = input.get("section").and_then(Value::as_str).unwrap_or("all");
        ...
        if section == "all" || section == "os" {
            ...
            if let Ok(hostname) = tokio::process::Command::new("hostname").output().await {
                info.push(format!("Hostname: {}", String::from_utf8_lossy(&hostname.stdout).trim()));
            }
        }

        if section == "all" || section == "cpu" {
            if let Ok(out) = tokio::process::Command::new("sysctl").args(["-n", "hw.ncpu"]).output().await {
                info.push(format!("CPU cores: {}", String::from_utf8_lossy(&out.stdout).trim()));
        ...
        if section == "all" || section == "disk" {
            if let Ok(out) = tokio::process::Command::new("df").args(["-h", "."]).current_dir(&ctx.working_dir).output().await {
        ...
        if section == "all" || section == "env_vars" {
            ...
            info.push(format!("HOME: {}", std::env::var("HOME").unwrap_or_default()));
            info.push(format!("USER: {}", std::env::var("USER").unwrap_or_default()));
            info.push(format!("SHELL: {}", std::env::var("SHELL").unwrap_or_default()));
        }
