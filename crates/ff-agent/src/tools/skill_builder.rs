...
impl AgentTool for SkillBuilderTool {
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let scope = input.get("scope").and_then(Value::as_str).unwrap_or("project");

        let skills_dir = if scope == "global" {
            dirs::home_dir().unwrap_or_default().join(".forgefleet").join("skills")
        } else {
        ...
        match action {
            "create" => {
                ...
                if let Some(params) = input.get("parameters").and_then(Value::as_array) {
                    ...
                    for param in params {
                        ...
                        let ptype = param.get("type").and_then(Value::as_str).unwrap_or("string");
                        let pdesc = param.get("description").and_then(Value::as_str).unwrap_or("");
                        let required = param.get("required").and_then(Value::as_bool).unwrap_or(false);
                        skill_md.push_str(&format!("- `{pname}` ({ptype}{}): {pdesc}\n",
                ...
                if !url.is_empty() {
                    ...
                    match client.get(url).send().await {
                        Ok(resp) if resp.status().is_success() => {
                            ...
                            let skill_dir = skills_dir.join(if name.is_empty() { "downloaded-skill" } else { name });
                            let _ = fs::create_dir_all(&skill_dir).await;
                    ...
                    AgentToolResult::err("Provide 'url' to install from, or use 'create' to make a new skill".to_string())
                }
            }
            ...
            "list" => {
                ...
                // Global skills
                let global_dir = dirs::home_dir().unwrap_or_default().join(".forgefleet").join("skills");
                if let Ok(mut entries) = fs::read_dir(&global_dir).await {
                    while let Ok(Some(entry)) = entries.next_entry().await {
                        if entry.path().is_dir() {
                            skills.push(format!("  [global] {}", entry.file_name().to_string_lossy()));
                        }
                    }
                }
                ...
                if let Ok(mut entries) = fs::read_dir(&project_dir).await {
                    while let Ok(Some(entry)) = entries.next_entry().await {
                        if entry.path().is_dir() {
                            skills.push(format!("  [project] {}", entry.file_name().to_string_lossy()));
                        }
                    }
                }

                if skills.is_empty() {
                    AgentToolResult::ok("No custom skills installed. Use SkillBuilder create to make one.".to_string())
                } else {
                    AgentToolResult::ok(format!("Custom Skills ({}):\n{}", skills.len(), skills.join("\n")))
                }
            }

            "show" => {
                let name = input.get("name").and_then(Value::as_str).unwrap_or("");
                if name.is_empty() { return AgentToolResult::err("'name' required"); }

                for dir in [&skills_dir, &dirs::home_dir().unwrap_or_default().join(".forgefleet").join("skills")] {
                    let skill_path = dir.join(name).join("SKILL.md");
                    if let Ok(content) = fs::read_to_string(&skill_path).await {
                        return AgentToolResult::ok(format!("Skill: {name}\nPath: {}\n\n{content}", skill_path.display()));
                    }
                }
            ...
            "test" => {
                let name = input.get("name").and_then(Value::as_str).unwrap_or("");
                if name.is_empty() { return AgentToolResult::err("'name' required for test"); }
                AgentToolResult::ok(format!("Skill test for '{name}': Use Bash tool to run the skill's command with test parameters."))
            }
