...
impl AgentTool for BudgetTrackerTool {
    ...
    fn description(&self) -> &str {
        "Track income and expenses. Categorize transactions, calculate totals, show spending breakdown."
    }
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let cat_lines: Vec<String> = cats
            .iter()
            .map(|(c, a)| format!("  {c}: ${a:.2} ({:.0}%)", a / expenses * 100.0))
            .collect();
...
impl AgentTool for ProfitLossTool {
    ...
    fn description(&self) -> &str {
        "Calculate profit & loss statement. Revenue, COGS, gross profit, operating expenses, net income."
    }
    fn parameters_schema(&self) -> Value {
...
impl AgentTool for CashFlowForecastTool {
    ...
    fn description(&self) -> &str {
        "Project future cash flow based on recurring income and expenses over N months."
    }
    fn parameters_schema(&self) -> Value {
        ...
        for m in 1..=months {
            ...
            for item in &one_time {
                if item.get("month").and_then(Value::as_u64) == Some(m as u64) {
                    ...
                    balance += amt;
                    lines.push(format!(
                        "  Month {m:>2}: ${balance:>12.2}  ({desc}: ${amt:+.2})"
                    ));
                    continue;
                }
            }
            if !lines
                .last()
                .map(|l: &String| l.contains(&format!("Month {m:>2}")))
                .unwrap_or(false)
            {
                lines.push(format!("  Month {m:>2}: ${balance:>12.2}"));
            }
        }
        let runway = if net_monthly < 0.0 && balance > 0.0 {
            format!("{:.0} months", balance / (-net_monthly))
        } else {
            "N/A".into()
        };
        AgentToolResult::ok(format!(
...
impl AgentTool for InvoiceGenTool {
    ...
    fn description(&self) -> &str {
        "Generate a professional invoice from line items. Outputs formatted markdown."
    }
    fn parameters_schema(&self) -> Value {
