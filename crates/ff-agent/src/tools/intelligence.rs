...
impl AgentTool for PatternLearnerTool {
    ...
    fn description(&self) -> &str {
        "Record successful patterns (tool sequences, approaches, fixes) so ForgeFleet learns over time what works for which task types."
    }
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        match action {
            "record" => {
                ...
                let mut patterns: Vec<Value> = fs::read_to_string(&patterns_file)
                    .await
                    .ok()
            ...
            "query" => {
                ...
                let patterns: Vec<Value> = fs::read_to_string(&patterns_file)
                    .await
                    .ok()
                    .and_then(|c| serde_json::from_str(&c).ok())
                    .unwrap_or_default();

                let matching: Vec<&Value> = patterns
                    .iter()
                    .filter(|p| {
                        task_type.is_empty()
                            || p.get("task_type").and_then(Value::as_str) == Some(task_type)
                    })
                ...
                if matching.is_empty() {
                    ...
                    let output: Vec<String> = matching
                        .iter()
                        .take(10)
                        .map(|p| {
                            format!(
                                "  [{}] {}",
                                p.get("task_type").and_then(Value::as_str).unwrap_or("?"),
                                p.get("pattern").and_then(Value::as_str).unwrap_or("")
                            )
                        })
                        .collect();
                    AgentToolResult::ok(format!(
                        "Learned patterns ({} total):\n{}",
                        matching.len(),
                        output.join("\n")
                    ))
                }
            ...
            "stats" => {
                ...
                let successes = patterns
                    .iter()
                    .filter(|p| p.get("success").and_then(Value::as_bool) == Some(true))
                    .count();
                AgentToolResult::ok(format!(
                    "Pattern stats: {total} total, {successes} successful ({:.0}% success rate)",
                    if total > 0 {
...
impl AgentTool for ModelScorecardTool {
    ...
    fn description(&self) -> &str {
        "Track and compare fleet model performance. Record quality scores per model per task type, view leaderboards."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        match action {
            "record" => {
                ...
                let mut scores: Vec<Value> = fs::read_to_string(&scores_file)
                    .await
                    .ok()
                    .and_then(|c| serde_json::from_str(&c).ok())
                    .unwrap_or_default();
            ...
            "leaderboard" => {
                let scores: Vec<Value> = fs::read_to_string(&scores_file)
                    .await
                    .ok()
                    .and_then(|c| serde_json::from_str(&c).ok())
                    .unwrap_or_default();

                let mut model_avgs: std::collections::HashMap<String, (f64, u32)> =
                ...
                let mut leaderboard: Vec<(String, f64)> = model_avgs
                    .into_iter()
                    .map(|(m, (sum, count))| (m, sum / count as f64))
                    .collect();
                leaderboard
                ...
                let lines: Vec<String> = leaderboard
                    .iter()
                    .enumerate()
                    .map(|(i, (m, avg))| format!("  {}. {m}: {avg:.1}/100", i + 1))
                    .collect();
                AgentToolResult::ok(format!("Model Leaderboard:\n{}", lines.join("\n")))
            }
            _ => AgentToolResult::ok(
                "Use: record (save score), leaderboard (view rankings), compare (model vs model)"
                    .to_string(),
            ),
        }
    }
...
impl AgentTool for ReviewQueueTool {
    ...
    fn description(&self) -> &str {
        "Queue completed work for human review. Track approvals, rejections, and feedback to improve agent quality."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        match action {
            "submit" => {
                ...
                let mut queue: Vec<Value> = fs::read_to_string(&queue_file)
                    .await
                    .ok()
                    .and_then(|c| serde_json::from_str(&c).ok())
                    .unwrap_or_default();
                ...
                let _ = fs::write(
                    &queue_file,
                    serde_json::to_string_pretty(&queue).unwrap_or_default(),
                )
                .await;
                AgentToolResult::ok(format!("Submitted for review: {title} (ID: {})", &id[..8]))
            }
            "list" => {
                let queue: Vec<Value> = fs::read_to_string(&queue_file)
                    .await
                    .ok()
                    .and_then(|c| serde_json::from_str(&c).ok())
                    .unwrap_or_default();
...
impl AgentTool for RollbackManagerTool {
    ...
    fn description(&self) -> &str {
        "Rollback all changes made during a session. Uses git to revert uncommitted changes or reset to a specific commit."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        match action {
            "preview" => {
                ...
                match output {
                    Ok(out) => AgentToolResult::ok(format!(
                        "Changes that would be rolled back:\n\n{}",
            ...
            "stash" => {
                let output = tokio::process::Command::new("git")
                    .args(["stash", "push", "-m", "ForgeFleet agent rollback"])
                    .current_dir(&ctx.working_dir)
                    .output()
                    .await;
                match output {
                    Ok(out) if out.status.success() => AgentToolResult::ok(
                        "Changes stashed. Use 'git stash pop' to restore.".to_string(),
                    ),
            ...
            "rollback" => {
                ...
                match output {
                    Ok(out) if out.status.success() => AgentToolResult::ok(format!(
                        "Rolled back all uncommitted changes to {target}"
                    )),
...
impl AgentTool for SmartSearchTool {
    ...
    fn description(&self) -> &str {
        "Search across everything: code, memories, chat history, docs, git history. Returns the most relevant results from all sources."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
...
impl AgentTool for WatchAndReactTool {
    ...
    fn description(&self) -> &str {
        "Set up a watcher that triggers agent actions on events: file changes, git pushes, cron schedules, or webhook calls."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        ...
        match action {
            ...
            "list" => AgentToolResult::ok(
                "Active watchers: (query fleet cron system for registered watchers)".to_string(),
            ),
            "delete" => {
                let id = input
                    .get("watcher_id")
...
impl AgentTool for ProjectScaffoldTool {
    ...
    fn description(&self) -> &str {
        "Generate a new project from templates: Rust CLI, Rust API (Axum), React app, Python FastAPI, Node Express, or custom."
    }
    fn parameters_schema(&self) -> Value {
        json!({"type":"object","properties":{
    ...
    async fn execute(&self, input: Value, ctx: &AgentToolContext) -> AgentToolResult {
        ...
        let cmd = match template {
            ...
            "rust-api" => format!(
                "cargo init --name {name} '{}' && cd '{}' && cargo add axum tokio serde serde_json",
                project_dir.display(),
                project_dir.display()
            ),
            "react-app" => format!(
                "npx create-vite@latest {name} --template react-ts -- --dir '{}'",
                base.display()
            ),
            "python-api" => format!(
                "mkdir -p '{}' && cd '{}' && python3 -m venv .venv && echo 'fastapi\\nuvicorn' > requirements.txt",
                project_dir.display(),
                project_dir.display()
            ),
            "node-express" => format!(
                "mkdir -p '{}' && cd '{}' && npm init -y && npm install express typescript @types/express",
                project_dir.display(),
                project_dir.display()
            ),
            "empty" => format!(
