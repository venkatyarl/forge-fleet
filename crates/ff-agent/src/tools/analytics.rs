...
impl AgentTool for StatsCalcTool {
    ...
    fn description(&self) -> &str {
        "Calculate statistics: mean, median, mode, std deviation, percentiles, min/max, correlation."
    }
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        let mut data: Vec<f64> = input
            ...
            .unwrap_or_default();
        if data.is_empty() {
            return AgentToolResult::err("No data provided");
        }
...
impl AgentTool for TimeSeriesAnalysisTool {
    ...
    fn description(&self) -> &str {
        "Analyze time series data: detect trends, calculate moving averages, identify outliers."
    }
    fn parameters_schema(&self) -> Value {
    ...
    async fn execute(&self, input: Value, _ctx: &AgentToolContext) -> AgentToolResult {
        let values: Vec<f64> = input
            ...
            .unwrap_or_default();
        if values.len() < 3 {
            return AgentToolResult::err("Need at least 3 data points");
        }
        ...
        let outliers: Vec<String> = values
            .iter()
            .enumerate()
            ...
            .map(|(i, v)| {
                format!(
                    "  {} = {v:.2} ({:.1}σ)",
                    labels.get(i).unwrap_or(&format!("{i}")),
                    (v - y_mean) / std_dev
                )
            })
            .collect();
