...
mod tests {
    ...
    #[test]
    fn use_local_lane_keeps_mechanical_local_until_max_tries_then_cloud() {
        // Mechanical (prefers_cloud=false), breaker closed: local for attempts
        // 0..LOCAL_LANE_MAX_TRIES, then escalate to cloud (the lane is
        // heartbeat-safe since #62/#792, so the old one-try cloud-bias is gone).
        let max_tries = ff_routing_policy::LOCAL_LANE_MAX_TRIES as i32;
        for attempt in 0..max_tries {
            assert!(
                use_local_lane(attempt, false, false),
                "attempt {attempt} stays on the cheap local lane"
            );
        }
        assert!(
            !use_local_lane(max_tries, false, false),
            "attempt {max_tries} escalates to cloud"
        );
