...
impl TemplateRegistry {
    pub fn new() -> Self {
        let mut registry = Self { templates: Vec::new() };
    ...
    fn load_builtins(&mut self) {
        ...
        self.templates.push(Template {
            id: "agent-rust-developer".into(), name: "Rust Developer Agent".into(),
            category: TemplateCategory::Agent, description: "Expert Rust agent with cargo/clippy/test tools".into(),
            version: "1.0.0".into(), author: "ForgeFleet".into(),
            tags: vec!["rust".into(), "systems".into(), "backend".into()],
            content: include_str!("agent_roles.rs").lines().take(5).collect::<Vec<_>>().join("\n"),
            install_path: ".forgefleet/agents/rust-developer.toml".into(),
        });

        self.templates.push(Template {
            id: "agent-security-auditor".into(), name: "Security Auditor Agent".into(),
            category: TemplateCategory::Agent, description: "Security review agent (read-only, no write access)".into(),
            version: "1.0.0".into(), author: "ForgeFleet".into(),
        ...
        self.templates.push(Template {
            id: "agent-test-writer".into(), name: "Test Writer Agent".into(),
            category: TemplateCategory::Agent, description: "Generates comprehensive unit and integration tests".into(),
            version: "1.0.0".into(), author: "ForgeFleet".into(),
        ...
        self.templates.push(Template {
            id: "workflow-code-review".into(), name: "Code Review Workflow".into(),
            category: TemplateCategory::Workflow,
            description: "3-stage review: code agent writes, test agent validates, review agent approves".into(),
            version: "1.0.0".into(), author: "ForgeFleet".into(),
        ...
        self.templates.push(Template {
            id: "workflow-consensus-coding".into(), name: "Consensus Coding Workflow".into(),
            category: TemplateCategory::Workflow,
    ...
    pub fn search(&self, query: &str) -> Vec<&Template> {
        ...
        let mut results: Vec<(&Template, f64)> = self.templates.iter()
            .filter_map(|t| {
                let mut score = 0.0;
                if t.name.to_ascii_lowercase().contains(&lower) { score += 10.0; }
                if t.description.to_ascii_lowercase().contains(&lower) { score += 5.0; }
                if t.tags.iter().any(|tag| tag.to_ascii_lowercase().contains(&lower)) { score += 8.0; }
                if t.id.to_ascii_lowercase().contains(&lower) { score += 6.0; }
    ...
    pub async fn install(&self, id: &str, project_dir: &Path) -> anyhow::Result<String> {
        let template = self.get(id).ok_or_else(|| anyhow::anyhow!("Template '{id}' not found"))?;
    ...
    pub fn list(&self) -> Vec<TemplateSummary> {
        self.templates.iter().map(|t| TemplateSummary {
            id: t.id.clone(),
            name: t.name.clone(),
            category: t.category,
