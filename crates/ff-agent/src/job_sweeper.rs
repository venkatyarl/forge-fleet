...
/// Decide research liveness from the heartbeat only. Session wall-clock age is
/// intentionally absent: a slow orchestrator may legitimately run for hours.
pub fn is_research_heartbeat_stale(
    last_heartbeat_at: Option<DateTime<Utc>>,
    now: DateTime<Utc>,
    stale_after: Duration,
) -> bool {
    last_heartbeat_at.is_some_and(|heartbeat| heartbeat < now - stale_after)
}
...
pub async fn sweep_stale(
    ...
    // rows stay `running` forever — no worker ever re-claims them. Heartbeats
    // distinguish that crashed process from a slow-but-live orchestrator.
    let research_cutoff = now - policy.research_stale_after;

    let sessions = sqlx::query_scalar::<_, sqlx::types::Uuid>(
        "UPDATE research_sessions
            SET status = 'failed',
                completed_at = NOW(),
                error = COALESCE(error, 'reaped by sweeper — orchestrator heartbeat went stale before synthesis')
    ...
    let reaped_ids = match sessions {
        Ok(ids) => {
            summary.research_sessions_failed = ids.len();
            ids
        }
        Err(e) => {
            tracing::warn!("pg sweep research_sessions: {e}");
            Vec::new()
        }
    };
    if !reaped_ids.is_empty() {
        let subtasks = sqlx::query(
            "UPDATE research_subtasks
                SET status = 'failed',
                    completed_at = NOW(),
                    error = COALESCE(error, 'reaped by sweeper — research orchestrator process died (stuck running)')
...
mod tests {
    ...
    #[test]
    fn research_staleness_uses_heartbeat_not_session_age() {
        let now = Utc::now();
