...
fn task_identifiers(task: &str) -> Vec<String> {
    let stoplist: HashSet<&'static str> = [
        ...
        "large",
        "small",
        // Generic test-intent words describe the kind of work, not a source
        // region. Keeping them makes nearly every line in a large cfg(test)
        // tail look relevant. Compound symbols remain intact tokens, so names
        // such as `test_specific_behavior` are still selected exactly.
        "test",
        "tests",
        "testing",
        "unit",
...
mod tests {
    ...
    #[test]
    fn codegen_task_identifiers_filter_generic_test_intent_but_keep_exact_symbols() {
        let task = "Add integration tests for cafe_parser and test_specific_behavior; unit testing must stay deterministic — UTF-8 café";

        let identifiers = task_identifiers(task);

        let parser_pos = identifiers
            .iter()
            .position(|identifier| identifier == "cafe_parser")
            .expect("named implementation retained");
        let helper_pos = identifiers
            .iter()
            .position(|identifier| identifier == "test_specific_behavior")
            .expect("exact test helper retained");
        assert!(parser_pos < helper_pos, "task order remains deterministic");
        assert!(!identifiers.iter().any(|identifier| matches!(
            identifier.as_str(),
            "test" | "tests" | "testing" | "unit" | "integration"
        )));
        assert_eq!(identifiers, task_identifiers(task));
    }
    ...
    fn codegen_regions_preserve_target_before_bounded_test_context() {
        ...
        for i in 0..100 {
            content.push_str(&format!("    fn unrelated_test_{i}() {{}}\n"));
        }
        content.push_str("    fn test_specific_behavior() {}\n");
        for i in 100..2_000 {
        ...
        let identifiers = task_identifiers(
            "Add integration tests for run_command_capture_cancellable and test_specific_behavior",
        );
        ...
        assert!(regions.contains("fn test_specific_behavior()"));
