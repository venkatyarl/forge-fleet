...
// Keep large mmap-backed weights reclaimable during startup. Some fleet user
// managers inherit an unlimited RLIMIT_MEMLOCK; paired with llama.cpp's
// `--mlock`, that pinned an entire 69.5 GiB GLM and pushed a 128 GiB host into
// the OOM killer while its smaller resident models were still running. Never
// raise a host's inherited limit: smaller nodes may have a much lower hard cap.
const SYSTEMD_MEMLOCK_CAP_BYTES: u64 = 16 * 1024 * 1024 * 1024;
...
async fn await_bounded_owned_startup<F, T>(
    guard: &mut LaunchOwnershipGuard,
    timeout: std::time::Duration,
    startup: F,
) -> Result<T, &'static str>
where
    F: std::future::Future<Output = T>,
{
    tokio::time::timeout(timeout, await_owned_startup(guard, startup))
        .await
        .map_err(|_| "startup health deadline expired")
}
...
async fn load_model_with_authority(
    ...
    // Kernel bind state is occupancy authority. Unknown listeners are foreign,
    // not stale, and must never be killed to make room for a model.
    let live_pids = pids_listening_on_port(port).await;
    if !live_pids.is_empty() {
        return Err(format!(
            "{worker_name}:{port} is occupied by listener pid(s) {live_pids:?}; \
    ...
    let health = await_bounded_owned_startup(
        &mut launch_guard,
...
fn launched_model_path(runtime: &str, args: &[String]) -> Option<String> {
    ...
    match runtime {
        "llama.cpp" | "mlx" => flag_value("--model"),
        "vllm" => args
            .iter()
...
fn listener_identity_matches(
    ...
    expected_pid == actual_pid
        ...
        && actual_model.is_some_and(|actual| model_paths_match(expected_model, actual))
}

async fn inspect_listener_identity(
    port: u16,
    launched_pid: u32,
    launched_start: &str,
    launched_model: &str,
    ...
    let listeners = pids_listening_on_port(port).await;
    if listeners.as_slice() != [launched_pid] {
        return Err(format!(
            "listener pid mismatch: launched {launched_pid}, kernel reports {listeners:?}"
        ));
    }
    let actual_start = process_start_marker(launched_pid)
        .ok_or_else(|| format!("listener pid {launched_pid} exited before activation"))?;
    let processes = list_local_processes().await;
    let actual = processes
        .iter()
        .find(|process| process.pid == launched_pid && process.port == Some(port));
    if !actual.is_some_and(|process| {
        listener_identity_matches(
            ...
            process.pid,
            &actual_start,
            process.model_path.as_deref(),
        )
    }) {
        return Err(format!(
            "listener process start/model identity does not match launched pid {launched_pid}"
        ));
    }
    Ok(())
}
...
async fn cleanup_launched_process(pid: u32, start: &str, port: u16, systemd_owned: bool) -> bool {
    ...
    if systemd_owned {
        ...
        let main_pid = systemd_main_pid(port).await;
        if !systemd_owns_launch(pid, main_pid) {
            tracing::error!(
                pid,
                ?main_pid,
                port,
                "systemd launch ownership unavailable or changed; retaining fence"
            );
            return false;
        }
        stop_systemd_unit(port).await;
...
fn systemd_owns_launch(expected_pid: u32, observed_main_pid: Option<u32>) -> bool {
    observed_main_pid == Some(expected_pid)
}
...
pub(crate) async fn pids_listening_on_port(port: u16) -> Vec<u32> {
...
/// Derive the `LD_LIBRARY_PATH` value for a systemd inference-server unit:
/// prepend the binary's own directory (so a co-located `libmtmd.so.0` resolves)
/// to the daemon's existing `LD_LIBRARY_PATH` (so a needed libdir the daemon
/// already carries — e.g. a GPU node's CUDA/ROCm path — is preserved). This is
/// the exact rule the manual-spawn path applies to `cmd.env(...)`; keeping them
/// identical is the point. Returns `None` only when `program` has no parent dir.
fn systemd_ld_library_path(program: &str, prev: &str) -> Option<String> {
    std::path::Path::new(program).parent().map(|p| {
        let bin_dir = p.display().to_string();
        if prev.is_empty() {
            bin_dir
        } else {
            format!("{bin_dir}:{prev}")
...
fn bounded_model_memlock_bytes(soft: u64, hard: u64) -> u64 {
    soft.min(hard).min(SYSTEMD_MEMLOCK_CAP_BYTES)
}

#[cfg(unix)]
fn current_model_memlock_limit_bytes() -> u64 {
    let mut limit = libc::rlimit {
        rlim_cur: 0,
        rlim_max: 0,
    };
    // SAFETY: `limit` is a valid, writable rlimit value. On failure, returning
    // zero fails safe by disabling weight pinning for the generated unit
    // instead of risking an inherited unlimited lock budget.
...
fn render_systemd_unit(
    ...
    let ld_env = ld_library_path
        .map(|path| format!("Environment=LD_LIBRARY_PATH={path}"))
        .unwrap_or_default();
    format!(
        "# Auto-generated by ff_agent::model_runtime on `ff model autoload`.\n\
         ...
         LimitMEMLOCK={memlock_limit_bytes}\n\
         Restart=on-failure\n\
         RestartSec=10\n\
...
async fn write_systemd_unit(
    ...
    // to stay in sync with the spawn logic — and, exactly like the manual
    // path, PREPEND the binary dir to the daemon's existing LD_LIBRARY_PATH
    // rather than replacing it, so a co-located lib (libmtmd.so.0) AND any
    // path the daemon already carries (e.g. a GPU node's CUDA/ROCm libdir)
    // both resolve. Dropping `prev` here was a silent divergence from the
    // spawn path that could lose a needed libdir on systemd-started servers.
    let prev_ld = std::env::var("LD_LIBRARY_PATH").unwrap_or_default();
    let ld_library_path = systemd_ld_library_path(program, &prev_ld);
    ...
    let unit = render_systemd_unit(
        ...
        current_model_memlock_limit_bytes(),
    );
...
mod tests {
    ...
    #[tokio::test]
    async fn production_owned_startup_has_a_hard_outer_deadline() {
        let calls = Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let action: CleanupAction = {
            let calls = calls.clone();
            Arc::new(move || {
        ...
        let mut guard = LaunchOwnershipGuard::new(action);
        let result = await_bounded_owned_startup(
            &mut guard,
            std::time::Duration::from_millis(10),
            std::future::pending::<()>(),
        )
        .await;
        assert_eq!(result, Err("startup health deadline expired"));
        drop(guard);
        tokio::task::yield_now().await;
        assert_eq!(calls.load(std::sync::atomic::Ordering::SeqCst), 1);
    }
    ...
    #[test]
    fn systemd_cleanup_ownership_is_fail_closed() {
        assert!(systemd_owns_launch(41, Some(41)));
        assert!(!systemd_owns_launch(41, Some(42)));
        assert!(!systemd_owns_launch(41, None));
    }
    ...
    fn systemd_ld_library_path_prepends_bindir_and_preserves_prev() {
        // Empty daemon env → just the binary's own dir (no trailing colon that
        // would inject an empty/CWD entry).
        assert_eq!(
            systemd_ld_library_path("/opt/llama.cpp/build/bin/llama-server", ""),
            Some("/opt/llama.cpp/build/bin".to_string())
        );
        // Non-empty daemon env (e.g. a GPU node's CUDA libdir) MUST be preserved
        // by prepending the binary dir — the faithful mirror of the manual spawn.
        assert_eq!(
            systemd_ld_library_path(
                "/opt/llama.cpp/build/bin/llama-server",
                "/usr/local/cuda/lib64"
            ),
            Some("/opt/llama.cpp/build/bin:/usr/local/cuda/lib64".to_string())
        );
        // A root-level program has no parent dir → None (no Environment= line).
        assert_eq!(systemd_ld_library_path("/", "/usr/lib"), None);
    }

    #[test]
    fn rendered_systemd_unit_caps_memlock_once_and_keeps_mlock_launch() {
        let unit = render_systemd_unit(
            "/opt/llama.cpp/build/bin/llama-server --model /models/glm.gguf --mlock",
            55000,
            std::path::Path::new("/var/log/forgefleet/model-55000.log"),
            Some("/opt/llama.cpp/build/bin:/usr/local/cuda/lib64"),
            SYSTEMD_MEMLOCK_CAP_BYTES,
        );

        assert_eq!(
            unit.lines()
                .filter(|line| *line == "LimitMEMLOCK=17179869184")
                .count(),
            1
        );
        assert!(!unit.contains("MemoryMax="));
        assert!(unit.contains(
            "ExecStart=/opt/llama.cpp/build/bin/llama-server --model /models/glm.gguf --mlock\n"
        ));
        assert!(unit.contains(
            "Environment=LD_LIBRARY_PATH=/opt/llama.cpp/build/bin:/usr/local/cuda/lib64\n"
        ));
    }

    #[test]
    fn model_memlock_cap_never_raises_inherited_limits() {
        let peer_limit = 16_571_240_448;
        let small_limit = 8 * 1024 * 1024;
        let four_gib = 4 * 1024 * 1024 * 1024;

        assert_eq!(
            bounded_model_memlock_bytes(u64::MAX, u64::MAX),
            SYSTEMD_MEMLOCK_CAP_BYTES
        );
        assert_eq!(
            bounded_model_memlock_bytes(peer_limit, peer_limit),
            peer_limit
        );
        assert_eq!(
            bounded_model_memlock_bytes(small_limit, small_limit),
            small_limit
        );
        assert_eq!(
            bounded_model_memlock_bytes(SYSTEMD_MEMLOCK_CAP_BYTES, four_gib),
            four_gib
        );
    }
    ...
    #[test]
    fn listener_identity_rejects_foreign_pid_pid_reuse_and_model_mismatch() {
        assert!(listener_identity_matches(
            41,
            "start-a",
            "/models/qwen/model.gguf",
            41,
            "start-a",
            Some("/models/qwen/model.gguf"),
        ));
        assert!(!listener_identity_matches(
            41,
            "start-a",
        ...
        assert!(!listener_identity_matches(
            ...
            41,
            "start-a",
            Some("/models/foreign/model.gguf"),
        ));
    }

    #[test]
    fn launched_model_identity_is_derived_for_each_runtime() {
        assert_eq!(
            launched_model_path(
                "llama.cpp",
                &[
        ...
        assert_eq!(
            launched_model_path("vllm", &["serve".into(), "/m/vllm".into()]).as_deref(),
            Some("/m/vllm")
        );
    }
}
