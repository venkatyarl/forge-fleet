//! Scheduled backup **restore-drill** — verifies that one exact Postgres backup
//! is actually *restorable*, not merely present on disk.
//!
...
pub async fn reserve_drill_run(
    ...
    sqlx::query(
        ...
        SELECT $1, b.id, b.file_name, b.database_kind, false, 'reserved',
          ...
          FROM backups b
         WHERE b.id = $2 AND b.database_kind = 'postgres'
    ...
    match fence {
        ...
        None => anyhow::bail!(
            "no postgres backup row with id {backup_id}; exact restore drill refuses fallback"
        ),
    }
}
...
impl RestoreDrillTick {
    ...
    /// Run one full drill against one immutable Postgres backup ID. Never
    /// selects an older/local fallback when the requested artifact is absent.
    ...
    async fn drill_inner(&self, backup_id: uuid::Uuid, run_id: uuid::Uuid) -> DrillOutcome {
        // 1) select exactly the caller-fenced Postgres backup row.
        let row: Option<(String, i64, String)> = match sqlx::query_as(
            "SELECT file_name, size_bytes, checksum_sha256 \
        ...
        let Some((file_name, recorded_bytes, checksum)) = row else {
            return DrillOutcome::failed(
                run_id,
                Some(backup_id),
                "",
    ...
    async fn alert_and_log(&self, outcome: &DrillOutcome) {
        ...
        if should_alert(outcome.success, days, STALE_DRILL_DAYS) {
            ...
            tracing::info!(
                ...
                "restore-drill: PASS — exact postgres backup is restorable"
            );
        }
    }
