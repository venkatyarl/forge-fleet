...
pub async fn handoff_stuck_tasks(pg: &PgPool) -> Result<usize, sqlx::Error> {
    ...
    let demoted = sqlx::query(
        r#"
        WITH stale AS (SELECT id FROM fleet_tasks WHERE status = 'running' AND last_heartbeat_at < NOW() - make_interval(secs => $1::int) AND handoff_count < $2 FOR UPDATE SKIP LOCKED)
    ...
    let _ = sqlx::query(
        r#"
        WITH exhausted AS (SELECT id FROM fleet_tasks WHERE status = 'running' AND last_heartbeat_at < NOW() - make_interval(secs => $1::int) AND handoff_count >= $2 FOR UPDATE SKIP LOCKED)
        UPDATE fleet_tasks AS t
           SET status       = 'failed',
               completed_at = NOW(),
               error        = 'exceeded MAX_HANDOFFS retries'
          FROM exhausted WHERE t.id = exhausted.id
        "#,
    )
    .bind(STUCK_AFTER_SECS as i32)
    .bind(MAX_HANDOFFS)
