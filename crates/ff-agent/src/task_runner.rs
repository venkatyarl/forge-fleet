...
/// Normalize caller-provided capability tags and add the capability that is
/// mechanically evident from a shell command before the row is inserted.
    ...
    fn enqueue_pretags_ff_commands_and_normalizes_explicit_tags() {
        ...
        assert_eq!(
            shell_task_capabilities(
                "ff model list",
                &[" leader ".into(), "ff".into(), "leader".into(), "".into()]
            ),
            vec!["ff", "leader"]
        );
