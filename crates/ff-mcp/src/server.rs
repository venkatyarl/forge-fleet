...
mod tests {
    ...
    impl PathGuard {
        fn replace(path: impl AsRef<std::ffi::OsStr>) -> Self {
            let original = std::env::var_os("PATH").unwrap_or_default();
            // SAFETY: callers hold process_env_lock for the guard's lifetime.
    ...
    async fn ssh_timeout_keeps_same_server_session_usable() {
        ...
        let _path_guard = PathGuard::replace(path);
        ...
        assert!(board.result.is_some(), "pm_board result: {board:?}");
        assert!(board.error.is_none(), "pm_board error: {board:?}");
        ...
        assert!(status.result.is_some(), "fleet_status result: {status:?}");
        assert!(status.error.is_none(), "fleet_status error: {status:?}");
