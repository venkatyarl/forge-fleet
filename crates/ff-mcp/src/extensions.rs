...
pub async fn list(cwd: &Path, federation_timeout_secs: u64) -> Value {
    ...
    for skill in ff_agent::skill_catalog::discover(cwd) {
        extensions.push(json!({
            "kind": "skill",
            "id": format!("skill/local/{}", skill.id),
            "name": skill.name,
    ...
    for tool in registry.list() {
        extensions.push(json!({
            "kind": "tool",
            "id": format!("tool/forgefleet/{}", tool.name),
            "name": tool.name,
    ...
    match ff_core::config::load_config_auto() {
        Ok((config, _)) => {
            ...
            for tool in snapshot.tools {
                extensions.push(json!({
                    "kind": "tool",
                    "id": format!("tool/{}/{}", tool.service, tool.name),
                    "name": tool.name,
                    "description": tool.description,
                    "source": tool.service,
        ...
        Ok(pool) => match ff_db::pg_list_agents(&pool, true).await {
            Ok(agents) => {
                for agent in agents {
                    extensions.push(json!({
                        "kind": "agent",
                        "id": format!("agent/{}/{}", agent.source, agent.name),
                        "name": agent.name,
