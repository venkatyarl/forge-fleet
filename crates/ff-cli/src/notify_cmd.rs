//! `forgefleet notify` — send fleet notifications through the configured
//! channel.
//!
...
//! the same sender the daemon's alert/upgrade ticks use
//! (`ff_agent::telegram::send_telegram_from_secrets`) so there is ONE
//! notification path, not a fork. If Telegram isn't fully configured the
//! send is a no-op (the underlying fn skips), and we say so rather than
...
pub async fn handle_notify(cmd: NotifyCommand) -> Result<()> {
    match cmd {
        ...
        NotifyCommand::Send {
            ...
            // silently skips when unconfigured). Delivery needs BOTH secrets —
            // reporting "sent" off the bot token alone would lie whenever the
            // chat id is missing and the send was actually skipped.
            let has_token = ff_db::pg_get_secret(&pool, "telegram_bot_token")
                .await
                .ok()
                .flatten()
                .is_some();
            let has_chat = ff_db::pg_get_secret(&pool, "telegram_chat_id")
                .await
                .ok()
                .flatten()
                .is_some();
            let configured = has_token && has_chat;
