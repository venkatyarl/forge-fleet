...
/// Backwards-compatible name for the task complexity classification.
pub type TaskComplexity = ComplexityFlag;
...
mod tests {
    ...
    fn flags_cross_cutting_and_chained_tasks_as_complex() {
        ...
        assert_eq!(
            TaskProcessor.assess_complexity(&prompt),
            ComplexityFlag::Complex
        );
        assert_eq!(
            TaskProcessor.assess_complexity(&command),
            ComplexityFlag::Complex
        );
    }
}
