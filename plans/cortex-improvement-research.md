...
7. **Cross-repo merged corpus view** — unified namespace across corpora (monorepo/workspace nav; multi-computer relevance). ✅ PARTIAL: `ff cortex find --all-corpora <q>` (`find_symbols_all_corpora`) searches EVERY indexed corpus at once, fan-in-ranked, each hit tagged with its repo. Cross-repo relationship traversal: `ff cortex callers/callees <sym> --all-corpora` (#577) resolves the name in EVERY corpus + unions callers/callees, each tagged with its corpus (validated: `callers new --all-corpora` = 419 across 5 repos). Caveat: unions per-corpus callers of same-named defs (calls edges are intra-corpus); does NOT resolve a call in repo A to a def only in repo B.
8. **Visualization layer** — interactive HTML/SVG/GraphML + Obsidian export (graphify-style). ✅ PARTIAL: `ff cortex export <symbol> [--depth --max-nodes --format graphml|dot]` (`export_neighborhood` + pure unit-tested `graph_to_graphml`/`graph_to_dot`) exports a symbol's call-graph neighborhood as GraphML (yEd/Gephi/Cytoscape) or DOT (`dot -Tsvg`). Corpus-wide / interactive-HTML / Obsidian-canvas are the remaining variants.
...
### P2 — future
11. Agentic graph traversal (MCTS/ReAct, RANGER/LocAgent). 12. Broader languages (Go/C/C++/C#/Ruby/Swift/Kotlin/…). 13. Build/config graph (`Cargo.toml`/`package.json` → `depends_on`/`builds_into`). ✅ Ingestion was already built (`deps.rs`: `code:crate`/`dep:package` nodes + `depends_on` edges). Surfaced two ways now: `ff cortex deps` (aggregate — each package + dependent count) and **`ff cortex deps <crate>`** (#565: per-crate FORWARD deps + REVERSE dependents; #573 adds --transitive full rebuild closure + --json). `builds_into` edges + cross-corpus dep rollup remain. 14. NL→Cypher/SQL query generation. 15. Token-budget subgraph selection (PageRank/learned relevance). 16. Static dead-code / god-symbol / circular-import alerts. ✅ `ff cortex alerts` (`cortex_alerts`): god symbols (fan-in hotspots) + mutual import cycles reliably; isolated-functions count is shipped but heuristic/caveated (macro/attribute/framework calls are invisible to a tree-sitter call graph, so "dead code" is unreliable — labeled review-not-delete).
