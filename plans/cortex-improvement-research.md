...
### P2 — future
11. Agentic graph traversal (MCTS/ReAct, RANGER/LocAgent). 12. Broader languages (Go/C/C++/C#/Ruby/Swift/Kotlin/…). 13. Build/config graph (`Cargo.toml`/`package.json` → `depends_on`/`builds_into`). ✅ Ingestion was already built (`deps.rs`: `code:crate`/`dep:package` nodes + `depends_on` edges). Surfaced two ways now: `ff cortex deps` (aggregate — each package + dependent count) and **`ff cortex deps <crate>`** (#565: per-crate FORWARD deps, internal-crate vs external-pkg, + REVERSE dependents = workspace rebuild blast radius). `builds_into` edges + cross-corpus dep rollup remain. 14. NL→Cypher/SQL query generation. 15. Token-budget subgraph selection (PageRank/learned relevance). 16. Static dead-code / god-symbol / circular-import alerts. ✅ `ff cortex alerts` (`cortex_alerts`): god symbols (fan-in hotspots) + mutual import cycles reliably; isolated-functions count is shipped but heuristic/caveated (macro/attribute/framework calls are invisible to a tree-sitter call graph, so "dead code" is unreliable — labeled review-not-delete).
